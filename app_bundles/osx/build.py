...
if not VERSION:
    print(
        "Warning: BUILD_VERSION environment variable not set. Defaulting to 0.0.0-dev."
    )
    VERSION = "0.0.0-dev"
else:
...
LAUNCHER_BINARY_PATH = os.path.abspath(
    "launcher"
)  # need to compile this first from launcher.c if its not already compiled
...
def build_app(icns_path):
    ...
    pyinstaller_cmd = ["pyinstaller", "--noconfirm", SPEC_FILE]
...
def copy_static_files():
    print(
        f"\U0001f4c1 Copying static files from:\n  {SOURCE_LANDING_DIR}\n➡️  to\n  {DEST_LANDING_DIR}"
    )
    if not os.path.exists(SOURCE_LANDING_DIR):
        raise FileNotFoundError(
            f"STATIC LANDING FILES SOURCE DIRECTORY NOT FOUND: {SOURCE_LANDING_DIR} not found!"
        )
    ...
    shutil.copytree(
        SOURCE_LANDING_DIR, DEST_LANDING_DIR, dirs_exist_ok=True, ignore=ignore_patterns
    )
...
def notarize_app(zip_path):
    ...
    cmd = [
        ...
        "--wait",
        "--output-format",
        "plist",  # or 'json' for structured output
    ]
    ...
    try:
        ...
        print(f"Notarization failed with exit code: {e.returncode}")
...
def create_dmg(app_path, dmg_path, skip_sign=False):
    ...
    unversioned_dmg_filename = (
        f"{APP_NAME.replace(' ', '-')}.dmg"  # Should be "Roboflow-Inference.dmg"
    )

    print(
        f"Running {script_path}. Expecting it to create {unversioned_dmg_filename}, which will then be ensured to be {dmg_path}."
    )
    ...
    if (
        os.path.exists(unversioned_dmg_filename)
        and unversioned_dmg_filename != dmg_path
    ):
        print(
            f"DMG created as {unversioned_dmg_filename}. Renaming to versioned name: {dmg_path}"
        )
        shutil.move(unversioned_dmg_filename, dmg_path)
    elif os.path.exists(dmg_path):
        print(
            f"DMG successfully found at versioned path: {dmg_path} (make_dmg.sh might be version-aware or previous rename succeeded)."
        )
    else:
        print(
            f"Error: DMG not found after running {script_path}. Expected {unversioned_dmg_filename} or {dmg_path}."
        )
        ...
        raise FileNotFoundError(
            f"DMG file not found. {script_path} did not produce {unversioned_dmg_filename} or {dmg_path}."
        )
...
def create_app_bundle_with_native_launcher(
    source_dir, app_bundle_path, launcher_binary_path, icon_path=None
):
...
if __name__ == "__main__":
    ...
    if skip_sign and not skip_notarize:
        print(
            "⚠️ Notarization requires a signed app. Skipping notarization because signing is skipped."
        )
        skip_notarize = True
    ...
    build_app(ICNS_PATH)
    copy_static_files()
    create_app_bundle_with_native_launcher(
        BUILD_DIR, APP_PATH, LAUNCHER_BINARY_PATH, icon_path=ICNS_PATH
    )
    ...
    if skip_sign and skip_notarize:
        ...
        print(
            f"🔏 Signing only with identity: {'ad-hoc (-)' if adhoc_sign else identity}"
        )
        sign_app_bundle(APP_PATH, identity)
