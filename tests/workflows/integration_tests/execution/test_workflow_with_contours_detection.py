...
def test_workflow_with_classical_contour_detection(
    ...
    assert not np.array_equal(result[0]["grayscale_image"].numpy_image, dogs_image), "Expected grayscale image to be different from input image"
    assert not np.array_equal(result[0]["blurred_image"].numpy_image, dogs_image), "Expected blur image to be different from input image"
    assert not np.array_equal(result[0]["thresholded_image"].numpy_image, dogs_image), "Expected threshold image to be different from input image"
    assert not np.array_equal(result[0]["contour_image"].numpy_image, dogs_image), "Expected contour outline image to be different from input image"
    assert (
