...
class FakeOBSClient:
    """Stands in for the obs-websocket client and records what the workflow asked for."""
...
def test_workflow_switches_obs_scene_on_detected_class(
    ...
    scene_calls = [c for c in fake_obs[0].calls if c[0] == "SetCurrentProgramScene"]
    ...
    assert scene_calls == [("SetCurrentProgramScene", {"sceneName": "Dogs"})]
