...
def test_pick_largest_perspective_polygons_raises_on_unexpected_type_of_input(broken_input):
...
def test_pick_largest_perspective_polygons_raises_on_unexpected_type_of_batch_element(broken_input):
...
def test_pick_largest_perspective_polygons_raises_on_empty_batch_element(empty_batch_elements):
    with pytest.raises(ValueError, match="Unexpected empty batch element"):
        pick_largest_perspective_polygons(perspective_polygons_batch=empty_batch_elements)
...
def test_pick_largest_perspective_polygons():
    ...
    # when
    largest_polygons = pick_largest_perspective_polygons(perspective_polygons_batch=batch)

    # then
    assert len(largest_polygons) == 2, "Output batch size must be the same as input batch size"
    assert np.array_equal(largest_polygons[0], large_perspective_pollygon), "Largest polygon must be picked"
    assert np.array_equal(largest_polygons[1], large_perspective_pollygon), "Largest polygon must be picked"


def test_sort_polygon_vertices_clockwise():
    # when
    clockwise_polygon = sort_polygon_vertices_clockwise(polygon=np.array([[1, 1], [1, 10], [10, 10], [10, 1]]))

    # then
    assert np.array_equal(clockwise_polygon, np.array([[1, 10], [1, 1], [10, 1], [10, 10]])), "Polygon vertices must be sorted clockwise"
...
def test_extend_perspective_polygon_detections_within_polygon():
    ...
    detections_on_the_corners = sv.Detections(xyxy=np.array([
        [99, 99, 101, 100],
        [109, 99, 111, 100],
        [99, 109, 101, 110],
        [109, 109, 111, 110]
    ...
    assert np.array_equal(
        extended_polygon,
        np.array([[100, 110], [100, 100], [110, 100], [110, 110]])
