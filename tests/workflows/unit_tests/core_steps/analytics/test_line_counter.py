...
def test_line_counter() -> None:
    ...
    frame1_detections = sv.Detections(
        xyxy=np.array([[10, 10, 11, 11], [20, 20, 21, 21], [100, 100, 101, 101], [200, 200, 201, 201]]),
        tracker_id=np.array([1, 2, 3, 4]),
    )
    frame2_detections = sv.Detections(
        xyxy=np.array([[20, 10, 21, 21], [10, 20, 11, 11], [90, 90, 91, 91], [5, 5, 6, 6]]),
...
def test_line_counter_no_trackers() -> None:
    ...
    # when
    with pytest.raises(ValueError, match="tracker_id not initialized, LineCounterBlockV1 requires detections to be tracked"):
...
def test_line_counter_too_short_line_segment() -> None:
    ...
    # when
    with pytest.raises(ValueError, match="LineCounterBlockV1 requires line zone to be a list containing exactly 2 points"):
        _ = line_counter_block.run(
            detections=detections,
            metadata=metadata,
            line_segment=line_segment,
...
def test_line_counter_line_segment_not_points() -> None:
    ...
    # when
    with pytest.raises(ValueError, match="LineCounterBlockV1 requires each point of line zone to be a list containing exactly 2 coordinates"):
        _ = line_counter_block.run(
            detections=detections,
            metadata=metadata,
            line_segment=line_segment,
...
def test_line_counter_line_segment_coordianates_not_numeric() -> None:
    ...
    # when
    with pytest.raises(ValueError, match="LineCounterBlockV1 requires each coordinate of line zone to be a number"):
        _ = line_counter_block.run(
            detections=detections,
            metadata=metadata,
            line_segment=line_segment,
