...
def test_run_locally_returns_inference_models_detections_with_model_id_output() -> None:
    # given
    ...
    block = RoboflowObjectDetectionModelBlockV2(
    ...
    # then
    # v2 manifest declares inference_id + predictions + model_id (same as v3).
    assert set(result[0].keys()) == {"inference_id", "predictions", "model_id"}
    assert isinstance(result[0]["predictions"], TensorDetections)
    assert result[0]["model_id"] == "m/1"


def test_run_remotely_returns_inference_models_detections_with_model_id_output() -> None:
    # given
    images = Batch(content=[_make_image()], indices=[(0,)])
    ...
    # when
    with patch(
        "inference.core.workflows.core_steps.models.roboflow.object_detection.v2_tensor.InferenceHTTPClient",
