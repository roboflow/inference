...
def test_kimi_openrouter_step_defaults_to_roboflow_managed_api_key() -> None:
    # given
    specification = {
        "type": "roboflow_core/kimi_openrouter@v1",
        "name": "step_1",
...
def test_execute_kimi_requests_when_rf_key_uses_roboflow_proxy(
    post_to_roboflow_api: MagicMock,
) -> None:
    # given
    post_to_roboflow_api.return_value = {
        "choices": [{"message": {"content": "This is proxied content from Kimi"}}]
    }

    # when
    result = execute_kimi_requests(
        roboflow_api_key="rf-workspace-key",
        kimi_api_key="rf_key:account",
        kimi_prompts=[[{"content": [{"text": "prompt"}]}]],
        model_version_id="moonshotai/kimi-k2.6",
        max_tokens=300,
        temperature=0.5,
    ...
    # then
    assert result == ["This is proxied content from Kimi"]
