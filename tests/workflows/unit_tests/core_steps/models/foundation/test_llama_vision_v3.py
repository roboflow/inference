"""Tests for the Llama 4 Vision v3 block: Llama 4 models, predictions decoded in-block.

The v2 behavior suite lives in ``test_llama_vision_v2.py``; this file
covers the v3 delta - the move to Llama 4 Scout / Maverick and the
``predictions`` / ``error_status`` / ``inference_id`` outputs.
"""
...
BASE_MANIFEST = {
    "type": "roboflow_core/llama_vision@v3",
    "name": "step",
    "images": "$inputs.image",
}
...
def _run(raw_output: str, task_type: str) -> dict:
    with patch.object(
        LlamaVisionBlockV3, "execute_openrouter_batch", return_value=[raw_output]
    ):
        block = LlamaVisionBlockV3(model_manager=MagicMock(), api_key="ws-key")
        results = block.run(
            images=[_image()],
            task_type=task_type,
            prompt="describe",
            ...
            privacy_level="deny",
            model_version="Llama 4 Scout - OpenRouter",
            max_tokens=128,
            temperature=0.1,
...
def test_manifest_parses_new_type() -> None:
    ...
    assert manifest.type == "roboflow_core/llama_vision@v3"
    assert manifest.task_type == "object-detection"


def test_manifest_offers_only_llama_4_models_and_defaults_to_scout() -> None:
    manifest = BlockManifest.model_validate(
        {**BASE_MANIFEST, "task_type": "unconstrained", "prompt": "describe"}
    )

    assert MODEL_VERSION_MAPPING == {
        "Llama 4 Scout - OpenRouter": "meta-llama/llama-4-scout",
        "Llama 4 Maverick - OpenRouter": "meta-llama/llama-4-maverick",
    }
    assert manifest.model_version == "Llama 4 Scout - OpenRouter"
