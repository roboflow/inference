...
class _FakeStreamingModel:
    ...
    def prompt(self, image, bboxes, state_dict=None, clear_old_points=True, frame_idx=0):
        self.calls.append(
            (
                "prompt",
                {
...
def _make_block_with_fake_model():
    ...
    block._model = fake
    block._current_model_id = "segment-anything-2-rt"
    return block, fake
...
def test_manifest_parses_valid_config():
    ...
    assert manifest.model_id == "segment-anything-2-rt"
    ...
    for frame_number in range(3):
        results.append(
            block.run(
                ...
                boxes=[boxes],
                model_id="segment-anything-2-rt",
                prompt_mode="first_frame",
                prompt_interval=30,
                threshold=0.0,
            )
