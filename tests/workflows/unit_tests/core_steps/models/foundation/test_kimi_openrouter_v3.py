"""Tests for the Kimi v3 block: predictions are decoded inside the block.

The v2 behavior suite lives in ``test_kimi_openrouter_v2.py``; this file
...
def _run(raw_output: str, task_type: str) -> dict:
    with patch.object(
        KimiOpenrouterBlockV3, "execute_openrouter_batch", return_value=[raw_output]
    ):
        block = KimiOpenrouterBlockV3(model_manager=MagicMock(), api_key="ws-key")
        results = block.run(
            images=[_image()],
            task_type=task_type,
            prompt="describe",
...
def test_manifest_parses_new_type() -> None:
    ...
    assert manifest.type == "roboflow_core/kimi_openrouter@v3"
