...
def test_scheduler_orders_cuda_work_across_submitter_and_worker_streams(
    ...
    """Scheduler events order producer, worker, and consumer CUDA streams."""
    module = types.ModuleType("tracktors")

    def update_batch(trackers, detections, **kwargs):
        """Delay and clone outputs on the scheduler worker stream."""
    ...
    try:
        with torch.cuda.stream(producer_stream):
            ...
            output = scheduler.update(_Tracker(), detections)
