...
def test_run_with_pixels_per_unit_converts_area():
    # given — 100x50 bbox = 5000 px², pixels_per_unit = 10 (so scale = 100)
    detections = sv.Detections(
        xyxy=np.array([[0, 0, 100, 50]], dtype=np.float32),
        mask=None,
    )

    # when
    block = MaskAreaMeasurementBlockV1()
    result = block.run(predictions=detections, pixels_per_unit=10.0)

    # then
    out = result[OUTPUT_KEY]
    assert out.data[AREA_KEY_IN_SV_DETECTIONS][0] == 5000.0
    assert out.data[AREA_CONVERTED_KEY_IN_SV_DETECTIONS][0] == 50.0  # 5000 / 100


def test_run_with_default_pixels_per_unit_areas_equal():
    # given
    detections = sv.Detections(
        xyxy=np.array([[0, 0, 20, 30]], dtype=np.float32),
        mask=None,
    )
    ...
    # then — default pixels_per_unit=1, so area_px == area_converted
    out = result[OUTPUT_KEY]
    assert out.data[AREA_KEY_IN_SV_DETECTIONS][0] == 600.0
    assert out.data[AREA_CONVERTED_KEY_IN_SV_DETECTIONS][0] == 600.0
