...
def test_run_calls_project_search_and_returns_best_match() -> None:
    ...
    assert result["match"]["image_id"] == "img-1"
    assert result["match"]["score"] == 1.82
    assert result["match"]["metadata"] == {"sku": "A-123"}
    assert "owner" not in result["match"]
    assert "raw" not in result["match"]
    assert result["matches"][1]["image_id"] == "img-2"
...
def test_run_batch_returns_one_result_per_image() -> None:
    ...
    assert [row["match"]["image_id"] for row in result] == ["img-1", "img-2"]
