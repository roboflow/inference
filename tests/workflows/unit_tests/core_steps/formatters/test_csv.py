...
from inference.core.workflows.core_steps.common.query_language.entities.operations import DetectionsPropertyExtract, \
    SequenceLength, StringToUpperCase
from inference.core.workflows.core_steps.formatters.csv.v1 import BlockManifest, unfold_parameters, prepare_csv_content
...
def test_prepare_csv_rows() -> None:
    ...
    columns_operations = {
        "batch_3": [StringToUpperCase(type="StringToUpperCase")]
    }
    ...
    # then
    assert set(result_df.columns) == {"some", "other", "batch_1", "batch_2", "batch_3", "timestamp"}, \
        "Expected all explicitly defined columns + timestamp one"
    assert result_df["some"].tolist() == ["value", "value", "value"], "Non batch data expected to be broadcast"
    assert result_df["other"].tolist() == [3, 3, 3], "Non batch data expected to be broadcast"
    assert result_df["batch_1"].tolist() == [1, 2, 3]
    assert result_df["batch_2"].tolist() == [True, True, True], "Single element batch expected to be broadcast"
    assert result_df["batch_3"].tolist() == ["A", "B", "C"], "Expected operation to be applied for each letter in each row"
