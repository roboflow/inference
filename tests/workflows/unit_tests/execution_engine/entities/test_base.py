...
def test_workflow_image_data_getting_lineage_when_lineage_not_set_and_only_parent_set() -> None:
    # given
    image = WorkflowImageData(
        parent_metadata=ImageParentMetadata(parent_id="parent"),
        numpy_image=np.zeros((192, 168, 3), dtype=np.uint8),
...
def test_workflow_image_data_getting_lineage_when_lineage_not_set_and_both_parent_and_root_set() -> None:
    # given
    image = WorkflowImageData(
        parent_metadata=ImageParentMetadata(parent_id="parent"),
        workflow_root_ancestor_metadata=ImageParentMetadata(parent_id="root"),
...
def test_workflow_image_data_getting_lineage_when_lineage_set_and_both_parent_and_root_set() -> None:
    # given
    image = WorkflowImageData(
        parent_metadata=ImageParentMetadata(parent_id="parent"),
        workflow_root_ancestor_metadata=ImageParentMetadata(parent_id="root"),
        numpy_image=np.zeros((192, 168, 3), dtype=np.uint8),
        lineage=["root", "pre-parent", "parent"],
    )
    ...
    # then
    assert result == ["root", "pre-parent", "parent"]