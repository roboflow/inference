...
from inference.models.perception_encoder.vision_encoder import config
...
def test_fetch_pe_checkpoint_uses_hugging_face_cache_in_offline_mode(
    ...
    with (
        patch.object(config, "HF_HUB_CACHE", expected_cache_path),
        patch.object(config, "OFFLINE_MODE", True),
    ):
...
def test_fetch_pe_checkpoint_preserves_online_download_tracking(hf_hub_download):
    ...
    with (
        patch.object(config, "HF_HUB_CACHE", expected_cache_path),
        patch.object(config, "OFFLINE_MODE", False),
    ):
        result = config.fetch_pe_checkpoint("PE-Core-B16-224")
