...
def test_service_urls_matrix_stays_in_sync_with_inference_sdk_mirror() -> None:
    # given
    from inference_sdk.regions import ROBOFLOW_SERVICE_URLS as SDK_ROBOFLOW_SERVICE_URLS

    # then
    assert ROBOFLOW_SERVICE_URLS == SDK_ROBOFLOW_SERVICE_URLS, (
        "inference.core.utils.regions deliberately mirrors inference_sdk.regions "
        "(inference core must not import inference_sdk at module level) - "
        "apply any matrix change to both copies."
    )
