...
def test_png16_round_trip_preserves_values_within_quantization_step():
    ...
    decoded = decode_png_normalized_depth(encode_normalized_depth_to_png16(depth))
...
def test_png16_encoding_accepts_array_like_input():
    decoded = decode_png_normalized_depth(
...
def test_png16_decoding_rejects_invalid_payload():
    with pytest.raises(ValueError):
        decode_png_normalized_depth("not-a-png")


def test_png8_round_trip_preserves_values_within_quantization_step():
    depth = np.linspace(0.0, 1.0, 64, dtype=np.float32).reshape(8, 8)

    decoded = decode_png_normalized_depth(encode_normalized_depth_to_png8(depth))

    assert decoded.shape == (8, 8)
    assert decoded.dtype == np.float32
    assert np.allclose(decoded, depth, atol=1.0 / 255)


def test_png8_round_trip_is_exact_at_bounds():
    depth = np.array([[0.0, 1.0], [1.0, 0.0]], dtype=np.float32)
...
def test_png8_encoding_clips_out_of_range_values():
    depth = np.array([[-0.5, 1.5]], dtype=np.float32)
...
def test_decode_handles_both_bit_depths_by_dtype():
    depth = np.array([[0.25, 0.75]], dtype=np.float32)

    decoded_16 = decode_png_normalized_depth(encode_normalized_depth_to_png16(depth))
    decoded_8 = decode_png_normalized_depth(encode_normalized_depth_to_png8(depth))

    assert np.allclose(decoded_16, depth, atol=1.0 / 65535)
    assert np.allclose(decoded_8, depth, atol=1.0 / 255)
