...
source = next(parent / 'inference/core/interfaces/camera/video_source.py'
...
namespace.update(VideoFrame=lambda **kw: SimpleNamespace(**kw),
...
exec(compile(ast.fix_missing_locations(ast.Module(body=[ast.ImportFrom(module='__future__', names=[ast.alias(name='annotations')], level=0), *selected], type_ignores=[])), str(source), 'exec'), namespace)
...
def test_eager_purge_records_each_discard_and_returns_same_latest():
    queue = Queue()
    for value in (11, 12, 13): queue.put(value)
    discarded, reads = [], []
    result = get_queue(queue, purge=True, on_discard=discarded.append,
...
def test_accounting_bounded_causes_and_source_local_counts():
    ...
    for event, payload in [('FRAME_CAPTURED', {'frame_id': 3}),
    ...
    for value in range(100):
        first(SimpleNamespace(event_type='FRAME_DROPPED', payload={'cause': str(value)}))
    result = first.snapshot()
    assert result['counts'] == {'captured': 1, 'enqueued': 1, 'returned': 1, 'retrieve_failed': 0}
...
def test_subsampling_emits_explicit_drop_without_retrieving():
    cls = next(n for n in tree.body if isinstance(n, ast.ClassDef) and n.name == 'VideoConsumer')
    method = next(n for n in cls.body if isinstance(n, ast.FunctionDef) and n.name == 'consume_frame')
    env = dict(namespace, FRAME_CAPTURED_EVENT='FRAME_CAPTURED', timedelta=__import__('datetime').timedelta)
    drops = []
    env['send_frame_drop_update'] = lambda **kw: drops.append(kw)
    exec(compile(ast.fix_missing_locations(ast.Module(body=[ast.ImportFrom(module='__future__', names=[ast.alias(name='annotations')], level=0),method],type_ignores=[])),str(source),'exec'),env)
    consumer = SimpleNamespace(_is_source_video_file=False, _timestamp_created=None,
        _frame_counter=0, _stream_consumption_pace_monitor=SimpleNamespace(tick=lambda:None, fps=30),
        _status_update_handlers=[], _video_fps_should_be_sub_sampled=lambda:True)
    assert env['consume_frame'](consumer,SimpleNamespace(grab=lambda:True),30,False,Queue(),True,0)
...
def test_source_accounting_is_opt_in_and_does_not_mix_shared_handlers(monkeypatch):
    ...
    cls = next(n for n in tree.body if isinstance(n, ast.ClassDef) and n.name == 'VideoSource')
    methods = [n for n in cls.body if isinstance(n, ast.FunctionDef)
               and n.name in ('__init__', 'frame_accounting')]
    stub = ast.ClassDef(name='SourceSubset', bases=[], keywords=[], body=methods, decorator_list=[])
    frame_type = type('Frame', (SimpleNamespace,), {})
    env = dict(namespace, os=os, Event=Event, VideoFrame=frame_type,
               sanitize_source_reference=lambda x:x, StreamState=SimpleNamespace(NOT_STARTED='idle'))
    exec(compile(ast.fix_missing_locations(ast.Module(body=[ast.ImportFrom(module='__future__', names=[ast.alias(name='annotations')], level=0),stub],type_ignores=[])),str(source),'exec'),env)
    ...
    def create():
        ...
        return env['SourceSubset']('safe',Queue(),shared,None,consumer,None,0),consumer
