...
def test_serialise_workflow_result() -> None:
    ...
    assert len(result_element["sixth"]) == 3, "Number of element in sixth list to be preserved"
    assert result_element["sixth"][:2] == ["some", 1], "First two elements not to be changed"
    assert result_element["sixth"][2][0] == 2, "First element of nested list not to be changed"
    assert (
