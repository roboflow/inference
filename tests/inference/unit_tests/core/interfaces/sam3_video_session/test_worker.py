...
def test_process_frames_checkpoints_every_n_samples_and_commits() -> None:
    ...
    frame_event = next(
        payload for event_type, payload in publisher.events if event_type == SAM3_VIDEO_EVENT_FRAME
    )
    assert (
...
def test_run_sam3_video_session_with_injected_frames_emits_downloading() -> None:
    publisher = FakePublisher()
    writer = FakeWriter()
