...
@pytest.mark.parametrize("category", ["model", "request", "workflows"])
def test_remote_preview_context_marks_usage_without_changing_billing(
    usage_collector_with_mocked_threads, category
):
    from inference_sdk.config import workflow_is_preview
    ...
    assert preview["resource_details"]["is_preview"] is True
    assert preview["resource_details"]["billable"] is True
    assert not extract()["resource_details"].get("is_preview", False)
