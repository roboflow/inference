...
def test_getting_blocks_descriptions(server_url) -> None:
    ...
    assert "declared_kinds" in response_data, "Declared kinds must be provided in output"
    assert len(response_data["declared_kinds"]) > 0, "Some kinds must be declared"
    assert "kinds_connections" in response_data, "Kinds connections expected to be declared"
    assert len(response_data["declared_kinds"]) >= len(response_data["kinds_connections"]), "Kinds connections declared as inputs for blocks must be at most in number of all declared kinds"
    assert "primitives_connections" in response_data, "Primitives connections expected to be in response"
    assert len(response_data["primitives_connections"]) > 0, "Expected some primitive parameters for steps to be declared"
