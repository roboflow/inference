...
def write_blocks_index_file(block_families):
    ...
    #using this to keep track of block cards for the index page
    block_card_lines = []

    for family_name, family_members in block_families.items():
        #aggregate some info about the family by looking at all blocks in the family
        ...
        # block card template for the block family on the index page
        block_card_lines.append(BLOCK_CARD_TEMPLATE.format(
...
def write_kinds_docs(blocks_description):
    ...
    for declared_kind in blocks_description.declared_kinds:
        ...
        #print(f"Writing kind docs for {declared_kind.name} to {kind_file_path}")
    ...
    #print(f"Writing kinds index to {KINDS_DOCUMENTATION_FILE}")
