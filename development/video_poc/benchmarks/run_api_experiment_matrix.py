...
def load_matrix(path):
    ...
    if len(source_selectors) != 1:
        raise ValueError("set exactly one of defaults.sourceId or defaults.sourceName")
...
def build_command(runner, matrix, scenario, run_id, output_dir, execute):
    ...
    for scenario in scenarios:
        for repetition in range(1, scenario["repetitions"] + 1):
            ...
            command = build_command(
                runner, matrix, scenario, run_id, output_dir, execute
            )
            started = utc_now()
...
def main(argv=None):
    ...
    try:
        ...
        api_key_env = matrix["defaults"].get(
            "apiKeyEnv", "VIDEO_BENCHMARK_API_KEY"
        )
        if args.execute and not os.environ.get(api_key_env):
