        ...
        *,
        ...
        if not secret:
            raise click.ClickException(
                "MODELS_SERVICE_INTERNAL_SECRET is required for staging registration."
            )
...
@click.option(
    "--staging-api-host",
    default=DEFAULT_STAGING_API_HOST,
    show_default=True,
    help="Staging Roboflow API host.",
)
@click.option(
    "--service-secret",
    default=None,
...
def main(
    trt_package_dir: Path,
    staging_api_host: str,
    service_secret: Optional[str],
    model_id: Optional[str],
    dry_run: bool,
) -> None:
    """Upload and seal a compiled TRT package on Roboflow staging."""
    ...
    click.echo("")
    click.echo("Registering model package on staging ...")
    try:
        ...
        if error.status_code == 409:
            raise click.ClickException(
                "Model package already exists for this manifest on staging."
            ) from error
    ...
    click.echo("")
    click.echo("Verify on staging:")
    click.echo("  export ROBOFLOW_ENVIRONMENT=staging")
    click.echo("  export ROBOFLOW_API_HOST=https://api.roboflow.one")
    click.echo(
