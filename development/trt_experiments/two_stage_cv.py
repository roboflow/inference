...
def main() -> None:
    ...
    for images_batch in decode_video_file(path=VIDEO_REFERENCE, batch_size=16):
        predictions = model.on_video_frame(frames=images_batch)
        model.on_prediction(predictions=predictions, frames=images_batch)


def decode_video_file(path: str, batch_size: int) -> Generator[List[np.ndarray], None, None]:
...
class TwoStageModel:
    ...
    def on_video_frame(self, frames: List[np.ndarray]) -> List[torch.Tensor]:
        ...
        frames = [torch.from_numpy(frame).to(DEVICE) for frame in frames]
        ...
        print(f"INFERENCE TIME: {round(dur * 1000, 2)} ms")
        return r

    def on_prediction(
        self,
        predictions: Union[List[Optional[AnyPrediction]], AnyPrediction],
        frames: Union[List[Optional[np.ndarray]], np.ndarray],
    ) -> None:
