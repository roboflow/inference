...
def main() -> None:
    pre_processing_queue = Queue(maxsize=32)
    post_processing_queue = Queue(maxsize=32)
    model = SingleStageModel(
        detector_path=DETECTOR_PATH,
        pre_processing_queue=pre_processing_queue,
        post_processing_queue=post_processing_queue,
    )
    inference_thread = Thread(target=model.inference_thread)
    inference_thread.start()
    post_processing_thread = Thread(target=model.post_processing_thread)
    post_processing_thread.start()
    pipeline = InferencePipeline.init_with_custom_logic(
        video_reference=[VIDEO_REFERENCE] * 16,
...
class SingleStageModel:
    ...
    def inference_thread(self) -> None:
        while True:
            pre_processed_images, images_metadata = self._pre_processing_queue.get()
            results = []
            for i in range(0, pre_processed_images.shape[0], DETECTOR_MAX_BATCH_SIZE):
                ...
                batch_results = perform_inference_from_detector(
                    ...
                    context=self._detector_context,
                    device=DEVICE,
                )
                results.append(batch_results)
            detections = torch.cat(results, dim=0)
            self._post_processing_queue.put((detections, images_metadata))
    ...
    def on_video_frame(self, frames: List[VideoFrame]) -> list:
        ...
        pre_processed_images, images_metadata = preprocess_images_for_detector(frames_tensors, (640, 640))
        self._pre_processing_queue.put((pre_processed_images, images_metadata))
        return [1] * len(frames)

    def post_processing_thread(self) -> None:
        while True:
            detections, images_metadata = self._post_processing_queue.get()
            if WITH_NMS:
            ...
            for _ in rescaled_detections:
                self._monitor.tick()
            print(f"FPS: {self._monitor.fps}")
