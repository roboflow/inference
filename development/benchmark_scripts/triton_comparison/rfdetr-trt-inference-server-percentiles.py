...
"""Benchmark RF-DETR TRT latency through Inference Server HTTP.

This script is intended to run inside the standard Roboflow GPU image. It
measures the HTTP endpoint end-to-end, including image encoding in the client,
server-side pre-processing, TRT inference, post-processing, response
serialization, and response JSON parsing.
...
Example:
    ...
    python development/benchmark_scripts/triton_comparison/rfdetr-trt-inference-server-percentiles.py \\
        ...
        --result-out /output/inference-server-benchmark.json
"""
