...
from inference.v1.models.yolov8.yolov8_object_detection_onnx import YOLOv8ForObjectDetectionOnnx
...
def main() -> None:
    ...
    _ = get_model(MODEL)
    model_package_path = os.path.join(MODEL_CACHE_DIR, resolve_roboflow_model_alias(MODEL))
    model = YOLOv8ForObjectDetectionOnnx.from_pretrained(model_package_path, device=torch.device(DEVICE))
