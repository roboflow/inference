...
def are_all_files_cached(files: List[Union[str, re.Pattern]], model_id: Optional[str] = None) -> bool:
    return all(is_file_cached(file=file, model_id=model_id) for file in files)


def is_file_cached(file: Union[str, re.Pattern], model_id: Optional[str] = None) -> bool:
...
def exists_file_matching_regex(file: re.Pattern, model_id: Optional[str] = None) -> bool:
    cache_dir = get_cache_dir(model_id=model_id)
