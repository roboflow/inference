...
def get_model_metrics(
    device_id: str, model_id: str, start: float = -1, stop: float = float("inf")
) -> dict:
    """
    ...
    """
    now = time.time()
    inferences_with_times = cache.zrange(
        f"inference:{device_id}:{model_id}", start=start, stop=stop, withscores=True
    )
    ...
    errors_with_times = cache.zrange(
        f"error:{device_id}:{model_id}", start=start, stop=stop, withscores=True
    )
