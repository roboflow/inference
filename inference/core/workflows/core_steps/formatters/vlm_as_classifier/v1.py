...
Visual Language Models (VLMs). Input is parsed to classification prediction which is output from the 
block.
...
def parse_multi_class_classification_results(
    ...
    try:
        ...
        logging.warning(
            f"Could not parse multi-class classification results in in `roboflow_core/vlm_as_classifier@v1` block. "
            f"Error type: {error.__class__.__name__}. Details: {error}"
        )
...
def parse_multi_label_classification_results(
    ...
    try:
        ...
        logging.warning(
            f"Could not parse multi-label classification results in in `roboflow_core/vlm_as_classifier@v1` block. "
            f"Error type: {error.__class__.__name__}. Details: {error}"
        )
        return {"error_status": True, "predictions": None, "inference_id": inference_id}
