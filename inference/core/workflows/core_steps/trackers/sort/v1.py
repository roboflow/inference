...
SHORT_DESCRIPTION = "Track and update object positions across video frames using SORT."
LONG_DESCRIPTION = """
Track objects across video frames using the SORT (Simple Online and Realtime Tracking)
algorithm from the roboflow/trackers package.

SORT uses IoU-based Hungarian assignment with Kalman filtering for single-stage association.
It is fast and lightweight, making it well suited for controlled environments with reliable
detections. It does not use appearance features, so identity switches may occur after
occlusions.
...
class SORTTrackerBlockManifest(WorkflowBlockManifest):
    ...
    type: Literal["roboflow_core/sort@v1"]
...
class SORTTrackerBlockV1(WorkflowBlock):
    ...
    def run(
        ...
        if not fps:
            ...
            logger.warning(
                ...
                "fps in order to initialise SORTTracker"
            )
        video_id = metadata.video_identifier
        if video_id not in self._trackers:
            self._trackers[video_id] = SORTTracker(
