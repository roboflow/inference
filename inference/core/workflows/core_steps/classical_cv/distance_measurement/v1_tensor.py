...
class DistanceMeasurementBlockV1(WorkflowBlock):
    ...
    def run(
        self,
        predictions: TensorNativeDetections,
...
def _iter_boxes_with_class_names(
    detections: TensorNativeDetections,
) -> List[Tuple[Tuple[int, int, int, int], str]]:
    """Yield ``((x_min, y_min, x_max, y_max), class_name)`` pairs for each native
    detection — the tensor-native equivalent of
...
def measure_distance_with_reference_object(
    ...
    reference_bbox = None
    for (x_min, y_min, x_max, y_max), class_name in _iter_boxes_with_class_names(
