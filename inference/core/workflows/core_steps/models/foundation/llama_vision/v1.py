...
Llama Vision 3.2 model is exposed via [OpenRouter API](https://openrouter.ai/).
...
class BlockManifest(WorkflowBlockManifest):
    ...
    api_key: Union[
        ...
        examples=["rf_key:account", "sk-or-...", "$inputs.llama_api_key"],
        private=True,
    )
    model_version: Union[
...
def execute_llama_vision_32_request(
    ...
    return execute_direct_openrouter_request(
        client=client,
        prompt=prompt,
        model_version_id=llama_model_version,
        max_tokens=max_tokens,
        temperature=temperature,
    )
