...
This block takes one or more images as input and runs them through a trained classification model. The model analyzes each image and assigns exactly one class label from its predefined set of classes. The block returns:
...
The model processes the entire image and outputs a single classification result per image, making it ideal for categorizing images into distinct, non-overlapping categories. Unlike multi-label classification (which can assign multiple tags), single-label classification selects exactly one class that best describes the image.
