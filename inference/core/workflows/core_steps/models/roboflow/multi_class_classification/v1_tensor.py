"""Tensor-native sibling of `roboflow_core/roboflow_classification_model@v1`.
...
The manifest mirrors the numpy ``v1`` sibling EXACTLY (3-way ``type`` Literal
with legacy aliases, plain ``confidence`` field, ``inference_id`` output kind
``STRING_KIND``, no ``model_id`` output) — ONLY the ``predictions`` output kind
