"""Tensor-native sibling of `roboflow_core/roboflow_instance_segmentation_model@v1`.
...
- LOCAL: ``ModelManager.run_tensor_native_inference`` returns
  ...
  ``torch.Tensor`` vs ``InstancesRLEMasks``) is adapter-decided: the v1 manifest
...
``.../instance_segmentation/v1.py``; this manifest is identical except the output
kinds.
"""
...
class RoboflowInstanceSegmentationModelBlockV1(WorkflowBlock):
    ...
    def run_locally(
        ...
        for image, detections in zip(images, detections_batch):
            ...
            # numpy v1 surfaces ``p.get(INFERENCE_ID_KEY)`` off the model dump);
