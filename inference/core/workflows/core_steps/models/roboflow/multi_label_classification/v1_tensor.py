"""Tensor-native sibling of `roboflow_core/roboflow_multi_label_classification_model@v1`.
...
This mirrors ``v3_tensor`` but reproduces the v1 manifest exactly: the legacy
type aliases, a flat ``confidence`` float threshold (no ``confidence_mode`` /
``custom_confidence`` / validator), and v1's reduced outputs (``predictions`` +
``inference_id`` only — NO ``model_id`` output, and ``inference_id`` is a plain
``STRING_KIND``). The result dicts likewise carry no ``model_id`` key.
