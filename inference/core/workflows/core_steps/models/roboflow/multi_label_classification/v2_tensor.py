"""Tensor-native sibling of `roboflow_core/roboflow_multi_label_classification_model@v2`.
...
This mirrors ``v3_tensor`` but reproduces the v2 manifest exactly: the single
``@v2`` type literal, a flat ``confidence`` float threshold (no
``confidence_mode`` / ``custom_confidence`` / validator), and v2's outputs
