"""Tensor-native sibling of ``roboflow_core/roboflow_keypoint_detection_model@v2``.
...
This is the v2 manifest (single ``@v2`` ``type`` literal; ``inference_id`` output is
``INFERENCE_ID_KIND``; a ``model_id`` output is present, and ``model_id`` is echoed in
each result dict) wired to the v3_tensor run bodies. v2 carries a plain ``confidence``
field (there is no ``confidence_mode`` / ``custom_confidence`` split - that was
introduced in v3), so ``confidence`` is forwarded directly.
