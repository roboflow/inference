...
``inference_models`` prediction so the shared tensor serialiser/consumers work
unchanged.
...
This reuses the exact RLE approach of ``semantic_segmentation/v2_tensor.py``.

KEY DIFFERENCE FROM v2_tensor (manifest only):
The v1 manifest has NO ``confidence_mode`` / ``custom_confidence`` params (and no
``validate`` validator); ``run`` takes only ``images`` + ``model_id``. Otherwise the
flow mirrors v2_tensor: LOCAL calls ``run_tensor_native_inference`` (->
``SemanticSegmentationResult``) and builds per-class RLE detections via
``_build_instance_detections_from_segmentation``; REMOTE converts the HTTP
``segmentation_mask`` / ``confidence_mask`` / ``class_map`` response via
...
serialiser ``serializers_tensor.py::serialise_sv_detections`` handles it unchanged.
...
guaranteed to be id 0 - its id is the index of ``background`` (case-insensitive) in
the model's class names / class_map. We therefore exclude:
  * the class id whose name is ``background`` (resolved from the response
    ``class_map``), and
...
whose background is not at index 0 are handled correctly - unlike numpy ``v1.py``,
which hardcodes ``cid != 0``.
...
(a) serializer registration (``KINDS_SERIALIZERS``, loader.py ~1139) - the kind
    ...
    ``serializers_tensor`` when ``ENABLE_TENSOR_DATA_REPRESENTATION`` is set). This
    is the SAME delta v2_tensor documents (the kind is shared by both versions):
...
(b) block import-swap (loader.py, mirroring the object_detection@v3 swap) - import
    the v1 tensor sibling under the flag:
...
class RoboflowSemanticSegmentationModelBlockV1(WorkflowBlock):
    ...
    def run_locally(
        ...
        class_names_map = _class_names_map(
            list(self._model_manager.get_class_names(model_id))
        )
...
def _class_names_map_from_class_map(class_map: Dict[str, str]) -> Dict[int, str]:
...
def _bbox_from_binary_mask(binary_mask: torch.Tensor) -> Optional[List[float]]:
    ...
    ``(H, W)`` torch bool mask, or ``None`` if empty (inclusive max, as numpy v1)."""
    rows = torch.where(torch.any(binary_mask, dim=1))[0]
    cols = torch.where(torch.any(binary_mask, dim=0))[0]
...
def _build_instance_detections_from_segmentation(
    ...
    """LOCAL path: dense ``(H, W)`` class grid + ``(H, W)`` confidence -> one RLE
    instance per present non-background/non-ignore class. Masks are encoded one
    class at a time (``torch_mask_to_coco_rle``); no ``N x H x W`` dense stack is
    materialised. Mirrors ``semantic_segmentation/v2_tensor.py``."""
...
def _build_instance_detections_from_inference_response(
    ...
    class_names_map = _class_names_map_from_class_map(class_map)
...
def _bbox_from_numpy_mask(binary_mask: np.ndarray) -> Optional[List[float]]:
    """Tight ``[x_min, y_min, x_max, y_max]`` enclosing all True pixels of a
    ``(H, W)`` numpy bool mask, or ``None`` if empty (mirrors numpy ``v1.py``)."""
