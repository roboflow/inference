...
class OverlapManifest(WorkflowBlockManifest):
    ...
    def describe_outputs(cls) -> List[OutputDefinition]:
        # Cache the OutputDefinition instance to avoid creating it every time the method is called.
        if not hasattr(cls, "_output_definition"):
            cls._output_definition = [
                OutputDefinition(
                    name=OUTPUT_KEY,
                    kind=[OBJECT_DETECTION_PREDICTION_KIND],
                ),
            ]
        return cls._output_definition
...
class OverlapBlockV1(WorkflowBlock):
    ...
    def run(
        ...
        filtered.data = {
            k: OverlapBlockV1.safe_trim(filtered.data[k], idx)
            for k in predictions.data.keys()
        }
