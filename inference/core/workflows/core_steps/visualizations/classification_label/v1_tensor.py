...
class ClassificationLabelManifest(ColorableVisualizationManifest):
    ...
    predictions: StepOutputSelector(kind=[TENSOR_NATIVE_CLASSIFICATION_PREDICTION_KIND]) = Field(  # type: ignore
...
class ClassificationLabelVisualizationBlockV1(ColorableVisualizationBlock):
    ...
    def run(
        ...
        predictions = to_legacy_classification_prediction(predictions)
        try:
...
def to_legacy_classification_prediction(
    prediction: Union[ClassificationPrediction, MultiLabelClassificationPrediction],
) -> dict:
    """Materialise a tensor-native classification prediction into the legacy
    ``dict`` shape the visualisation layout helpers consume.

    This is the single, sanctioned native -> dict conversion for this block: the
    visualiser output is an annotated image (never a native prediction), the
    ``sv.LabelAnnotator`` is fed a ``pseudo_detections`` built from per-class
    ``class_id`` / ``confidence`` / ``class`` fields (not from the prediction
    object), and every layout helper below reads the dict keys the numpy block
    produced. So the rest of ``run`` (and all module helpers) stay verbatim.

    Reconstructed shapes mirror the numpy classification blocks:

    * single-label (``ClassificationPrediction``) ->
      ``{"image": {"width", "height"}, "predictions": [{"class", "class_id",
      "confidence"}, ...]}`` over the full softmax distribution; class names come
      from ``images_metadata[0]["class_names"]`` (``{int class_id: str name}``),
    * multi-label (``MultiLabelClassificationPrediction``) ->
      ``{"image": {"width", "height"}, "predictions": {class_name:
      {"confidence", "class_id"}, ...}, "predicted_classes": [...]}``; class
    ...
    if isinstance(prediction, ClassificationPrediction):
        ...
        predictions = [
            ...
            for class_id in range(int(confidence.shape[-1]))
        ]
        return {
    ...
    class_ids = (
        prediction.class_ids.detach().cpu().numpy().tolist()
        if prediction.class_ids is not None
        else []
    )
    num_classes = int(confidence.shape[-1])
    predictions = {
        _resolve_class_name(class_id, class_names_mapping): {
            "confidence": float(confidence[class_id]),
            "class_id": int(class_id),
        }
    ...
    predicted_classes = [
        _resolve_class_name(int(class_id), class_names_mapping)
...
def _resolve_image_dimensions(image_metadata: dict) -> Tuple[int, int]:
    image_dimensions = image_metadata.get(IMAGE_DIMENSIONS_KEY)
    if image_dimensions is None:
        raise ValueError(
            "Classification prediction is missing image dimensions required to "
            "place labels."
        )
    height, width = int(image_dimensions[0]), int(image_dimensions[1])
    return height, width
