...
Unlike v1 which counts frames, v2 reasons about real time.
...
- Use **v1** when you want to keep exactly N most recent frames regardless of
  timing.
- Use **v2** when you want a buffer that reflects "the last K seconds of
  video," automatically sized to the source FPS, with optional subsampling
  to reduce CPU/token cost.
"""
...
class BlockManifest(WorkflowBlockManifest):
    model_config = ConfigDict(
        json_schema_extra={
            "name": "Time-Windowed Image Stack",
            "version": "v2",
    ...
    type: Literal["roboflow_core/image_stack@v2"]
...
class ImageStackBlockV2(WorkflowBlock):
    ...
    def run(
        ...
        if _read_source_fps(image) is None and not state.warned_no_fps:
            logger.warning(
                ...
                f"defaulting to {DEFAULT_FPS_FALLBACK} fps for ImageStack v2 "
