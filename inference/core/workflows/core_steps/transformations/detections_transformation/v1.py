...
from copy import copy
from typing import Any, Dict, List, Literal, Optional, Type, Callable
...
class BlockManifest(WorkflowBlockManifest):
    ...
    operations_parameters: Dict[str, Selector()] = Field(
        description="References to additional parameters that may be provided in runtime to parameterize operations",
        examples=[{"classes": "$inputs.classes"}],
        default_factory=lambda: {},
    )
    ...
    @classmethod
    def get_parameters_accepting_batches_and_scalars(cls) -> List[str]:
        # Allows mixing of batch-based parameters and scalar-based parameters in operations_parameters
        return ["operations_parameters"]
...
class DetectionsTransformationBlockV1(WorkflowBlock):
    """Block that applies transformations to bounding box detections (filtering, resizing, etc.)."""
    ...
    def run(
        ...
        """Executes the transformations on a batch of detections."""
        return execute_transformation(
            predictions=predictions,
            operations=operations,
            operations_parameters=operations_parameters,
...
def execute_transformation(
    ...
    """
    Applies a chain of transformations to a batch of detections.

    Args:
        predictions (Batch[sv.Detections]): Batch of predictions to be transformed.
        operations (List[OperationDefinition]): A list of operation definitions describing how to transform.
        operations_parameters (Dict[str, Any]): Mapping of parameter names to values, possibly batch-aligned.
        custom_comparators (Optional[Dict[str, Callable]]): Custom comparator functions keyed by comparator string.

    Returns:
        BlockResult: The transformed detection results, one per batch element.
    """
    if DEFAULT_OPERAND_NAME in operations_parameters:
        raise ValueError(
            f"Detected reserved parameter name: {DEFAULT_OPERAND_NAME} declared in "
            f"`operations_parameters` of `DetectionsTransformation` block."
        )

    # Build the operations chain, injecting any custom comparators
    operations_chain = build_operations_chain(
        operations=operations,
        custom_comparators=custom_comparators
    )

    # Separate batch-aligned parameters from scalar parameters
    batch_parameters = grab_batch_parameters(
        operations_parameters=operations_parameters,
        main_batch_size=len(predictions),
    )
    ...
    # Align everything for iteration:
    batches_to_align = [predictions] + [batch_parameters[k] for k in batch_parameters_keys]

    results = []
    for payload in zip(*batches_to_align):
        # The first element is detections, the rest are batch-aligned parameters
        detections = payload[0]
        single_eval_params = copy(non_batch_parameters)
        for key, value in zip(batch_parameters_keys, payload[1:]):
            single_eval_params[key] = value

        transformed_detections = operations_chain(
            detections,
            global_parameters=single_eval_params,
        )

        if not isinstance(transformed_detections, sv.Detections):
            raise ValueError(
                "Definition of operation chain provided to `DetectionsTransformation` block "
                f"transforms sv.Detections into an unsupported type: {type(transformed_detections)}"
            )
