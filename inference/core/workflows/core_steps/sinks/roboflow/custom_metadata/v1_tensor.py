...
class RoboflowCustomMetadataBlockV1(WorkflowBlock):
    ...
    def run(
        ...
        inference_ids: List[str] = _extract_inference_ids(predictions)
...
def _extract_inference_ids(
    ...
    # metadata, rather than the per-detection `sv.Detections.data[INFERENCE_ID_KEY]`
    # array the numpy sibling read from.
