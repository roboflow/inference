...
class PredictionsAggregator(object):
    ...
    def __init__(self):
        self._raw_predictions: dict[str, List[TensorNativePrediction]] = {}

    def collect(self, value: TensorNativePrediction, model_id: str) -> None:
...
def format_predictions_for_model_monitoring(
    predictions_list: List[TensorNativePrediction],
    model_id: str,
) -> List[ParsedPrediction]:
    results = []
    for predictions in predictions_list:
        if isinstance(predictions, ClassificationPrediction):
            # Tensor-native single-label classification: `confidence` is the full
            # softmax distribution `(bs, num_classes)` and class names live in
            # `images_metadata[i][CLASS_NAMES_KEY]`. The numpy sibling iterated the
            # `predictions` list (one entry per class) of the serialised dict, so we
            # emit one `ParsedPrediction` per class with its distribution confidence.
            images_metadata = predictions.images_metadata or []
            class_ids = predictions.class_id.detach().cpu().tolist()
            for image_index in range(len(class_ids)):
                image_metadata = (
                    images_metadata[image_index]
                    if image_index < len(images_metadata)
                    else {}
                )
        ...
        elif isinstance(predictions, MultiLabelClassificationPrediction):
            # Tensor-native multi-label classification: `confidence` is the full
            # sigmoid distribution `(num_classes,)` and class names live in
            # `image_metadata[CLASS_NAMES_KEY]`. The numpy sibling iterated the
            # `predictions` dict (one entry per class) of the serialised dict.
            ...
            # Detections / InstanceDetections / keypoint-detection tuple. Iterate the
            # native 7-tuple; the numpy sibling read per-detection `class_name`,
            # `confidence`, `inference_id` and `prediction_type` from
            # `sv.Detections.data[...]`. Tensor-native keeps the class-id -> name map,
            # `inference_id` and `prediction_type` per-image in `image_metadata`, so we
            # resolve `class_name` via that mapping and confidence from the tensor.
            _key_points, detections = split_key_point_prediction(predictions)
