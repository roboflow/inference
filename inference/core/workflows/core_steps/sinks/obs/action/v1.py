...
from inference.core.workflows.core_steps.common.detections_fields import (
...
class OBSActionBlockV1(WorkflowBlock):
    ...
    def _available_targets_hint(
        ...
        if "not found" not in text and "no source" not in text and "600" not in text:
            return ""
        ...
        def _describe(client: Any) -> str:
            ...
            if action != SET_SCENE and scene_name and scene_name in scenes:
                items = [
                    i["sourceName"]
                    for i in client.get_scene_item_list(scene_name).scene_items
                ]
                parts.append(f"sources in '{scene_name}': {', '.join(items[:20])}")
            elif action in (SET_TEXT, TOGGLE_FILTER):
                inputs = [i["inputName"] for i in client.get_input_list(None).inputs]
