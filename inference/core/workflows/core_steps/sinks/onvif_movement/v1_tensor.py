...
# Builds a native-empty `Detections` carrying over the source `image_metadata`
# (the tensor-native analogue of `sv.Detections.empty()`). Shared-helper
# candidate — the maintainer may consolidate this into `tensor_native`.
def _empty_detections_like(
...
class ONVIFSinkBlockV1(WorkflowBlock):
    ...
    def run(
        ...
        if movement_type == "Follow":
            ...
            if len(predictions.xyxy) == 0:
                ...
                return {
                    PREDICTIONS_OUTPUT_KEY: _empty_detections_like(predictions),
            ...
            # tracker_id lives in per-detection bboxes_metadata for tensor-native
            # predictions (numpy used predictions.tracker_id).
            bboxes_metadata = predictions.bboxes_metadata or [
                {} for _ in range(len(predictions))
            ]
            tracker_ids = [
                box_metadata.get(TRACKER_ID_KEY) for box_metadata in bboxes_metadata
            ]

            # if there's a tracked object, continue to use it
            if tracked_object:
                tracked_mask = [
                    tracker_id == tracked_object for tracker_id in tracker_ids
                ]
                tracked_predictions = take_prediction_by_mask(predictions, tracked_mask)
                if len(tracked_predictions.xyxy) > 0:
                    max_confidence_prediction = take_prediction_by_indices(
                        tracked_predictions, [0]
                    )

            # if there's no tracked object, use the max confidence prediction
            if max_confidence_prediction is None:
                max_confidence = predictions.confidence.max()
                max_confidence_index = int(
                    torch.nonzero(predictions.confidence == max_confidence)[0]
                )
                max_confidence_prediction = take_prediction_by_indices(
                    predictions, [max_confidence_index]
                )
                # if we're not tracking at the moment, start tracking this one
                max_confidence_tracker_id = (
                    max_confidence_prediction.bboxes_metadata or [{}]
                )[0].get(TRACKER_ID_KEY)
                if follow_tracker and max_confidence_tracker_id is not None:
                    tracked_object = int(max_confidence_tracker_id)
    ...
    def move_camera(
        ...
        prediction: Union[Detections, InstanceDetections],
        zoom_if_able: bool,
        dead_zone: int,
        flip_x_movement: bool,
        flip_y_movement: bool,
        ...
        # get dimensions of image and bounding box from prediction. For
        # tensor-native predictions root_parent_dimensions is a single
        # (height, width) in image_metadata (numpy stored an (N, 2) array
        # in prediction.data and indexed the first row).
        image_dimensions = (prediction.image_metadata or {})[ROOT_PARENT_DIMENSIONS_KEY]
        xyxy = prediction.xyxy.detach().to("cpu").numpy()

        # calculate centers
        x1, y1, x2, y2 = tuple(xyxy[0])
        image_height, image_width = tuple(image_dimensions)
