...
from inference.core.exceptions import (MissingApiKeyError, MissingDefaultModelError, MalformedRoboflowAPIResponseError,
...
from inference.enterprise.stream_manager.controllers.entities import OperationStatus, ErrorType, TYPE_KEY, CommandType, \
    STATUS_KEY, ERROR_TYPE_KEY
...
class InferencePipelineManager(Process):
    ...
    def run(self) -> None:
        while not self._stop:
            command: Tuple[str, dict] = self._command_queue.get()
    ...
    def _handle_command(self, request_id: str, payload: dict) -> None:
        try:
            ...
            self._handle_error(request_id=request_id, error=error, error_type=ErrorType.INVALID_PAYLOAD)
        except Exception as error:
            self._handle_error(request_id=request_id, error=error, error_type=ErrorType.INTERNAL_ERROR)
    ...
    def _initialise_pipeline(self, request_id: str, payload: dict) -> None:
        try:
            ...
            source_buffer_filling_strategy, source_buffer_consumption_strategy = None, None
            if "source_buffer_filling_strategy" in payload:
                source_buffer_filling_strategy = BufferFillingStrategy(payload["source_buffer_filling_strategy"].upper())
            ...
            self._handle_error(request_id=request_id, error=error, error_type=ErrorType.AUTHORISATION_ERROR)
        except RoboflowAPINotNotFoundError as error:
            self._handle_error(request_id=request_id, error=error, error_type=ErrorType.NOT_FOUND)
        except (MissingDefaultModelError, RoboflowAPIRequestError, MalformedRoboflowAPIResponseError) as error:
            self._handle_error(request_id=request_id, error=error, error_type=ErrorType.INTERNAL_ERROR)
    ...
    def _terminate_pipeline(self, request_id: str) -> None:
        ...
        try:
            ...
            self._handle_error(request_id=request_id, error=error, error_type=ErrorType.OPERATION_ERROR)

    def _mute_pipeline(self, request_id: str) -> None:
        if self._inference_pipeline is None:
            return self._handle_error(request_id=request_id, error_type=ErrorType.OPERATION_ERROR)
        try:
    ...
    def _handle_error(self, request_id: str, error: Optional[Exception] = None, error_type: ErrorType = ErrorType.INTERNAL_ERROR):
        logger.error(f"Could not handle Command. request_id={request_id}, error={error}, error_type={error_type}")
        response_payload = describe_error(error, error_type=error_type)
        self._responses_queue.put((request_id, response_payload))


def describe_error(exception: Optional[Exception] = None, error_type: ErrorType = ErrorType.INTERNAL_ERROR) -> dict:
    payload = {
...
def assembly_pipeline_sink(sink_config: dict) -> Callable[[ObjectDetectionPrediction, VideoFrame], None]:
