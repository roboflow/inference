...
def mesh_developable_reg(mesh):
    ...
    for iC in range(3):  # loop over three corners of each triangle
        ...
        corner_angle = torch.acos(
            torch.clamp(torch.sum(vAn * vBn, dim=-1), min=-1.0, max=1.0)
        )
        ...
        vert_normal_covariance_sum = torch_scatter.scatter_add(
            src=contrib, index=tris[:, iC], dim=-2, out=vert_normal_covariance_sum
        )

        vert_area = torch_scatter.scatter_add(
            src=face_area / 3.0, index=tris[:, iC], dim=-1, out=vert_area
        )

        vert_degree = torch_scatter.scatter_add(
            src=torch.ones(F, dtype=torch.int32, device=device),
            index=tris[:, iC],
            dim=-1,
            out=vert_degree,
        )
    ...
    vert_normal_covariance_sum = (
        vert_normal_covariance_sum + torch.eye(3, device=device)[None, :, :] * REL_EPS
    )

    min_eigvals = torch.min(
        torch.linalg.eigvals(vert_normal_covariance_sum).abs(), dim=-1
    ).values

    # Mask out degree-3 vertices
    vert_area = torch.where(
        vert_degree == 3,
        torch.tensor(0, dtype=vert_area.dtype, device=vert_area.device),
        vert_area,
    )
