...
def render_mesh_paper(
    mesh, mv, mvp, iter_res, return_types=["mask", "depth"], white_bg=False
):
    """
    The rendering function used to produce the results in the paper.
    """
    v_pos_clip = util.xfm_points(
        mesh.vertices.unsqueeze(0), mvp
    ...
    for type in return_types:
        if type == "mask":
            img = dr.antialias(
                (rast[..., -1:] > 0).float(), rast, v_pos_clip, mesh.faces.int()
            )
        elif type == "depth":
            ...
            normal_indices = (
                torch.arange(0, mesh.nrm.shape[0], dtype=torch.int64, device="cuda")[
                    :, None
                ]
            ).repeat(1, 3)
            img, _ = util.interpolate(
                mesh.nrm.unsqueeze(0).contiguous(), rast, normal_indices.int()
            )
        elif type == "vertex_normal":
            img, _ = util.interpolate(
                mesh.v_nrm.unsqueeze(0).contiguous(), rast, mesh.faces.int()
            )
...
class SplitVisualizer:
    ...
    def render(self, camera):
        lh_outputs = render_mesh(
            ...
            return_types=["normals", "wireframe"],
        ...
        outputs = {
            ...
            for k in ["normals", "wireframe"]
        }
        return {
            "img": (outputs["wireframe"] * ((outputs["normals"] + 1.0) / 2.0) * 255).to(
                torch.uint8
            ),
