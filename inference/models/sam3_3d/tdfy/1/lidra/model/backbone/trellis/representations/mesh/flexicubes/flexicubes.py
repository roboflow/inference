...
import torch
from .tables import *
# from kaolin.utils.testing import check_tensor  # Commented out to remove kaolin dependency
...
class FlexiCubes:
    ...
    def __call__(
        ...
        assert torch.is_tensor(voxelgrid_vertices), "'voxelgrid_vertices' should be a tensor of shape (num_vertices, 3)"
        # and check_tensor(voxelgrid_vertices, (None, 3), throw=False)  # Commented out to remove kaolin dependency
        num_vertices = voxelgrid_vertices.shape[0]
        assert torch.is_tensor(scalar_field), "'scalar_field' should be a tensor of shape (num_vertices,)"
        # and check_tensor(scalar_field, (num_vertices,), throw=False)  # Commented out to remove kaolin dependency
        assert torch.is_tensor(cube_idx), "'cube_idx' should be a tensor of shape (num_cubes, 8)"
        # and check_tensor(cube_idx, (None, 8), throw=False)  # Commented out to remove kaolin dependency
        num_cubes = cube_idx.shape[0]
        assert beta is None or torch.is_tensor(beta), "'beta' should be a tensor of shape (num_cubes, 12)"
        # and check_tensor(beta, (num_cubes, 12), throw=False)  # Commented out to remove kaolin dependency
        assert alpha is None or torch.is_tensor(alpha), "'alpha' should be a tensor of shape (num_cubes, 8)"
        # and check_tensor(alpha, (num_cubes, 8), throw=False)  # Commented out to remove kaolin dependency
        assert gamma_f is None or torch.is_tensor(gamma_f), "'gamma_f' should be a tensor of shape (num_cubes,)"
        # and check_tensor(gamma_f, (num_cubes,), throw=False)  # Commented out to remove kaolin dependency
    ...
    def _compute_reg_loss(self, vd, ue, edge_group_to_vd, vd_num_edges):
        ...
        ) / vd_num_edges.squeeze(1).to(vd.dtype)
        mad = (
    ...
    def _linear_interp(self, edges_weight, edges_x):
        ...
        return ue.to(edges_weight.dtype)
    ...
    def _compute_vd(
        ...
        """
        if voxelgrid_colors is not None:
            vd_color = torch.zeros(
                (total_num_vd, C), device=self.device, dtype=beta_sum.dtype
            )
