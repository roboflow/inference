...
if __name__ == "__main__":
    ...
    for it in range(FLAGS.iter):
        ...
        mv, mvp = render.get_random_camera_batch(
            FLAGS.batch, iter_res=FLAGS.train_res, device=device, use_kaolin=False
        )
        # render gt mesh
        target = render.render_mesh_paper(gt_mesh, mv, mvp, FLAGS.train_res)
        # extract and render FlexiCubes mesh
        grid_verts = x_nx3 + (2 - 1e-8) / (FLAGS.voxel_grid_res * 2) * torch.tanh(
            deform
        )
        ...
        depth_loss = (
            (
                (((buffers["depth"] - (target["depth"])) * target["mask"]) ** 2).sum(-1)
                + 1e-8
            )
        ).sqrt().mean() * 10

        t_iter = it / FLAGS.iter
        sdf_weight = FLAGS.sdf_regularizer - (
            FLAGS.sdf_regularizer - FLAGS.sdf_regularizer / 20
        ) * min(1.0, 4.0 * t_iter)
        reg_loss = (
            loss.sdf_reg_loss(sdf, grid_edges).mean() * sdf_weight
        )  # Loss to eliminate internal floaters that are not visible
        ...
        if it % FLAGS.save_interval == 0 or it == (
            FLAGS.iter - 1
        ):  # save normal image for visualization
            with torch.no_grad():
                ...
                mv, mvp = render.get_rotate_camera(
                    it // FLAGS.save_interval,
                    iter_res=FLAGS.display_res,
                    device=device,
                    use_kaolin=False,
                )
                val_buffers = render.render_mesh_paper(
                    flexicubes_mesh,
                    mv.unsqueeze(0),
                    mvp.unsqueeze(0),
                    FLAGS.display_res,
                ...
                val_image = (
                    (val_buffers["normal"][0].detach().cpu().numpy() + 1) / 2 * 255
                ).astype(np.uint8)

                gt_buffers = render.render_mesh_paper(
                    gt_mesh,
                    mv.unsqueeze(0),
                    mvp.unsqueeze(0),
                    FLAGS.display_res,
                ...
                gt_image = (
                    (gt_buffers["normal"][0].detach().cpu().numpy() + 1) / 2 * 255
                ).astype(np.uint8)
                imageio.imwrite(
                    os.path.join(FLAGS.out_dir, "{:04d}.png".format(it)),
                    np.concatenate([val_image, gt_image], 1),
                )
                print(
                    f"Optimization Step [{it}/{FLAGS.iter}], Loss: {total_loss.item():.4f}"
                )
