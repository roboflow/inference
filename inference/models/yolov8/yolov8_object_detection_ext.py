...
class Yolo8ODExperimentalModel(RoboflowInferenceModel):
    """Adapter for Yolov8 Object Detection using inference_exp AutoModel backend.
    ...
    def __init__(self, model_id: str, api_key: str = None, **kwargs) -> None:
        ...
        logger.info(f"Initialized YoloV8ODExperimentalModel for model_id: {model_id}")
    ...
    def infer(
        ...
        **kwargs,
        ...
        logger.info(
            f"YoloV8ODExperimentalModel Running inference for {len(image) if isinstance(image, list) else 1} images"
        )
        ...
        detections_list = self._exp_model(
            np_images[0], conf_thresh=float(confidence), iou_thresh=float(iou_threshold)
        )
        elapsed_ms = (perf_counter() - t0) * 1000.0
        logger.error(
            f"YoloV8ODExperimentalModel backend call took {elapsed_ms:.2f} ms {self._exp_model._device}"
        )
