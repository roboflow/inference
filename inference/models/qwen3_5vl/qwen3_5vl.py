...
class Qwen3_5VL(TransformerModel):
    generation_includes_input = True
    transformers_class = Qwen3_5ForConditionalGeneration
    processor_class = AutoProcessor
    default_dtype = torch.bfloat16
    skip_special_tokens = True

    default_system_prompt = (
        "You are a Qwen3.5-VL model that can answer questions about any image."
    )
...
class LoRAQwen3_5VL(LoRATransformerModel):
    ...
    default_system_prompt = (
        "You are a Qwen3.5-VL a helpful assistant for any visual task."
    )
    ...
    def initialize_model(self, **kwargs):
        ...
        if not model_load_id.startswith("qwen-pretrains/"):
            self.model = (
