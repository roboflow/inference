...
class PositionEmbeddingLearned(nn.Module):
    ...
    def forward(self, tensor_list: NestedTensor):
        ...
        # Precompute indices only once to avoid repeat torch.arange calls
        device = x.device
        ...
        # Avoid repeat(). Use efficient broadcasting with expand to build pos
        # x_emb_bcast: (h, w, c)
        x_emb_bcast = x_emb.unsqueeze(0).expand(h, w, -1)
        y_emb_bcast = y_emb.unsqueeze(1).expand(h, w, -1)
        pos = torch.cat([x_emb_bcast, y_emb_bcast], dim=-1)  # (h, w, 2c)

        # Direct broadcast to (h, w, bs, 2c) without repeat by unsqueeze/expand
        pos = pos.unsqueeze(2).expand(h, w, bs, -1)
        # return: (H, W, bs, C)
        return pos
