...
class Dinov2WithRegistersSelfAttention(nn.Module):
    def __init__(self, config: WindowedDinov2WithRegistersConfig) -> None:
        ...
        if hidden_size % num_heads != 0 and not hasattr(config, "embedding_size"):
            raise ValueError(
        ...
        all_head_size = num_heads * attention_head_size

        self.num_attention_heads = num_heads
        self.attention_head_size = attention_head_size
        self.all_head_size = all_head_size

        # Use out_features argument directly, avoids repeated lookups and provides small performance improvement.
    ...
    def transpose_for_scores(self, x: torch.Tensor) -> torch.Tensor:
        # Fastest memory layout for attention: use reshape/transposition instead of view/permute
        nhead = self.num_attention_heads
        head_dim = self.attention_head_size
        # x: (batch, seq, hidden). Reshape to (batch, seq, nhead, head_dim)
        x = x.reshape(x.shape[0], x.shape[1], nhead, head_dim)
        # Transpose to (batch, nhead, seq, head_dim)
