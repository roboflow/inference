...
class Dinov2WithRegistersSelfAttention(nn.Module):
    ...
    def transpose_for_scores(self, x: torch.Tensor) -> torch.Tensor:
        # Fast, memory-efficient view+transpose using reshape and transpose instead of view + permute
        shape = x.size()
        # shape: [batch, seq_len, hidden_size] -> [batch, seq_len, num_heads, head_size]
        x = x.reshape(
            shape[0], shape[1], self.num_attention_heads, self.attention_head_size
        )
