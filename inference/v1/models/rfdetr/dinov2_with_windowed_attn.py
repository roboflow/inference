...
def drop_path(
    ...
    # Perform allocation and computation in-place (vectorized)
    return _drop_path_inplace(input, keep_prob)


@torch.jit.script
def _drop_path_inplace(input: torch.Tensor, keep_prob: float) -> torch.Tensor:
    # Helper function for fast inplace dropout.
    # Assumes input requires grad.
    # (Assumes keep_prob is < 1.0. Callsite handles return if keep_prob==1.0)
    # - Mutates and returns new tensor.
    # - No Python-side branching
    shape = (input.shape[0],) + (1,) * (input.ndim - 1)
    # Use input.new_empty for fastest tensor allocation
    rand = torch.rand(shape, dtype=input.dtype, device=input.device)
    rand.add_(keep_prob).floor_()  # in-place add and binarize
    rand.div_(keep_prob)  # in-place scale to preserve activations
    return input * rand
...
class Dinov2WithRegistersDropPath(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        # Forward pass with optimized drop_path function.
