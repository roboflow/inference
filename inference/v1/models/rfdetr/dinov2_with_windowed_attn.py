...
def drop_path(
    ...
    # Efficient shape broadcasting using expand_as
    shape = (input.shape[0],) + (1,) * (input.ndim - 1)
    rand = torch.rand(shape, dtype=input.dtype, device=input.device)
    rand.add_(keep_prob).floor_()
    # In-place multiplication for memory efficiency. Only .div_() is safe, as out-of-place mul would allocate new memory.
    # Avoid modifying input in-place unless it is certain input is not reused, so we use out-of-place below.
    return input * rand.div(keep_prob)
