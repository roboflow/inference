...
class MultiheadAttention(nn.Module):
    ...
    def __init__(
        ...
        self.out_proj = NonDynamicallyQuantizableLinear(
            self.vdim, self.vdim, bias=bias, **factory_kwargs
        )
...
def multi_head_attention_forward(
    ...
    if static_k is None:
        k = (
            k.contiguous()
            .view(bsz, -1, num_heads, head_dim)
            .permute(0, 2, 1, 3)
            .contiguous()
            .view(bsz * num_heads, -1, head_dim)
        )
    else:
    ...
    if static_v is None:
        v = (
            v.contiguous()
            .view(bsz, -1, num_heads, v_head_dim)
            .permute(0, 2, 1, 3)
            .contiguous()
            .view(bsz * num_heads, -1, v_head_dim)
        )
    else:
        # TODO finish disentangling control flow so we don't do in-projections when statics are passed
        assert (
