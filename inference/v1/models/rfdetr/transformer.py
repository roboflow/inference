...
def gen_encoder_output_proposals(
    ...
    nlevel = spatial_shapes.shape[0]
    # Precompute level-wise wh scales to save repeated computation
    wh_scales = [0.05 * (2.0**lvl) for lvl in range(nlevel)]
    ...
    for lvl, (H_, W_) in enumerate(spatial_shapes):
        ...
        if memory_padding_mask is not None:
            ...
            # Use cached tensor w/ correct dtype
            valid_H = torch.full((N_,), H_, dtype=torch.float32, device=device)
            valid_W = torch.full((N_,), W_, dtype=torch.float32, device=device)

        # Build mesh grid (optimized)
        grid = _make_grid_cat(H_, W_, device)  # H_, W_, 2
        # N_ x H_ x W_ x 2, expand via .expand below
        scale = (
            torch.stack([valid_W, valid_H], dim=1).unsqueeze(1).unsqueeze(1)
        )  # (N_, 1, 1, 2)
        # Use broadcasting
        grid = (grid.unsqueeze(0) + 0.5) / scale

        wh = torch.full_like(grid, wh_scales[lvl])
        proposal = torch.cat((grid, wh), -1).view(N_, -1, 4)
        proposals.append(proposal)
        _cur += H_ * W_

    output_proposals = torch.cat(proposals, 1)  # (N_, S, 4)
    # Proposals are valid if all coordinates are in (0.01, 0.99)
    output_proposals_valid = (
        (output_proposals > 0.01) & (output_proposals < 0.99)
    ).all(-1, keepdim=True)

    # Mask and unsigmoid logic, vectorized as much as possible
    if unsigmoid:
        out_prop = torch.log(output_proposals / (1 - output_proposals))  # unsigmoid
        if memory_padding_mask is not None:
            # mask out all pads with +inf before validity mask to reduce scatter
            out_prop = out_prop.masked_fill(
                memory_padding_mask.unsqueeze(-1), float("inf")
            )
        out_prop = out_prop.masked_fill(~output_proposals_valid, float("inf"))
    else:
        out_prop = output_proposals
        if memory_padding_mask is not None:
            out_prop = out_prop.masked_fill(memory_padding_mask.unsqueeze(-1), float(0))
        out_prop = out_prop.masked_fill(~output_proposals_valid, float(0))
    ...
    # maintain dtype
    return output_memory.to(memory.dtype), out_prop.to(memory.dtype)
...
def _make_grid_cat(H_, W_, device):
    # Efficiently create mesh grid and stack for xy in broadcasted fashion
    grid_y = torch.arange(H_, dtype=torch.float32, device=device)
    grid_x = torch.arange(W_, dtype=torch.float32, device=device)
...
def _valid_HW(mask_flatten_, H_, W_):
    # vectorized valid height and width computation
    # mask_flatten_: N_, H_, W_, 1, dtype=bool
    N_ = mask_flatten_.shape[0]
