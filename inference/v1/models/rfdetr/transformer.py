...
def gen_encoder_output_proposals(
    ...
    # Precompute mesh grids and wh per level to avoid repeated construction per batch
    device = memory.device
    dtype = torch.float32

    all_grids = []
    all_whs = []
    # Precompute linspace for all levels to avoid recreation in loop for batch
    hws = spatial_shapes.tolist()  # [(h0,w0), (h1,w1), ...]
    for lvl, (H_, W_) in enumerate(hws):
        grid_y = torch.arange(H_, device=device, dtype=dtype)
        grid_x = torch.arange(W_, device=device, dtype=dtype)
        grid_y, grid_x = torch.meshgrid(grid_y, grid_x, indexing="ij")
        grid = torch.stack((grid_x, grid_y), dim=-1)  # H_, W_, 2
        # always add 0.5 in normalization phase
        grid = grid + 0.5
        all_grids.append(grid)  # grid, not expanded yet
        # wh (constant per level)
        wh = (0.05 * (2.0**lvl)) * torch.ones((H_, W_, 2), device=device, dtype=dtype)
        all_whs.append(wh)

    proposals_list = []
    valid_H_list = []
    valid_W_list = []
    batch_indices = torch.arange(N_, device=device)
    ...
    for lvl, (H_, W_) in enumerate(hws):
        ...
        if memory_padding_mask is not None:
            mask_flatten_ = memory_padding_mask[:, _cur : (_cur + level_len)].view(
                N_, H_, W_, 1
            )
        ...
        valid_H_list.append(valid_H)
        valid_W_list.append(valid_W)

        grid = all_grids[lvl]  # H_, W_, 2, already shifted +0.5
        wh = all_whs[lvl]  # H_, W_, 2

        scale = torch.stack((valid_W, valid_H), dim=1).view(N_, 1, 1, 2)  # N_,1,1,2
        grid = (
            grid.unsqueeze(0) / scale
        )  # (1, H_, W_, 2) / (N_,1,1,2) -> (N_, H_, W_, 2) by broadcast

        grid = grid.expand(N_, -1, -1, -1)
        wh = wh.unsqueeze(0).expand(N_, -1, -1, -1)
    ...
    # Proposal transformation (unsigmoid, masking)
    if unsigmoid:
        # avoid division by zero on locations where mask will fill with inf
        eps = 1e-6
        masked_output_proposals = torch.where(
            output_proposals_valid,
            torch.clamp(output_proposals, min=eps, max=1 - eps),
            torch.ones_like(output_proposals) * 0.5,
        )
        output_proposals = torch.log(
            masked_output_proposals / (1 - masked_output_proposals)
        )  # unsigmoid
        if memory_padding_mask is not None:
    ...
    # Merge two mask passes for output_memory to one, more efficient in-place
    output_memory = memory
    mask = None
    if memory_padding_mask is not None:
        mask = memory_padding_mask.unsqueeze(-1)
    prop_mask = ~output_proposals_valid

    if mask is not None:
        output_memory = output_memory.masked_fill(mask | prop_mask, 0.0)
    else:
        output_memory = output_memory.masked_fill(prop_mask, 0.0)
