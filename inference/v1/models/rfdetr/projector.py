...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        # Avoid recomputing x-u and unnecessary pow
        mean = x.mean(1, keepdim=True)
        xm = x - mean
        # Use `var = mean(xm^2)` to avoid extra temp tensors
        var = (xm * xm).mean(1, keepdim=True)
        # In-place normalization to minimize new allocations and maximize fusion
        xnorm = xm / torch.sqrt(var + self.eps)
        # Efficient broadcasting: add shape only if necessary
