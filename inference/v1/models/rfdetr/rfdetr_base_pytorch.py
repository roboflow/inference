...
from inference.v1.models.rfdetr.transformer import build_transformer
...
class LWDETR(nn.Module):
    ...
    def __init__(
        ...
        query_dim = 4
        emb_size = num_queries * group_detr

        self.refpoint_embed = nn.Embedding(emb_size, query_dim)
        self.query_feat = nn.Embedding(emb_size, hidden_dim)
        nn.init.zeros_(self.refpoint_embed.weight)
        ...
        if self.two_stage:
            # Avoid deepcopy, use share-weight MLP and Linear prototype
            enc_bbox_embed_prots = nn.ModuleList(
                [
                    type(self.bbox_embed)(*self.bbox_embed._get_name_args())
                    for _ in range(group_detr)
                ]
            )
            enc_class_embed_prots = nn.ModuleList(
                [
                    type(self.class_embed)(
                        self.class_embed.in_features, self.class_embed.out_features
                    )
                    for _ in range(group_detr)
                ]
            )
            for e in enc_bbox_embed_prots:
                e.load_state_dict(self.bbox_embed.state_dict())
            for e in enc_class_embed_prots:
                e.load_state_dict(self.class_embed.state_dict())
            self.transformer.enc_out_bbox_embed = enc_bbox_embed_prots
            self.transformer.enc_out_class_embed = enc_class_embed_prots
