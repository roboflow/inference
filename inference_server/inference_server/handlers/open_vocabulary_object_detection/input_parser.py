...
def _collect_classes(query_list: list[str], body_value: Any) -> list[str]:
    ...
    if isinstance(body_value, list):
        return [str(v) for v in body_value]
    return [str(body_value)]


async def parse_open_vocabulary_object_detection_input(
    ...
    classes = _collect_classes(
        request.query_params.getlist("classes"),
        extra_params.pop("classes", None),
    )
    if not classes:
        raise InputParseError(
