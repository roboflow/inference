...
def test_trt_package_torch_multiple_predictions_in_row(
    ...
    for _ in range(5):
        _ = model(coins_counting_image_torch)
    for _ in range(8):
        pre_processed, metadata = model.pre_process(coins_counting_image_torch)
        raw_predictions = model.forward(pre_processed)
        # Finish the output concatenation before postprocessing switches streams.
        torch.cuda.synchronize(pre_processed.device)
