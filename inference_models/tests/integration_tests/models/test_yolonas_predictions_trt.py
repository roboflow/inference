...
def test_trt_package_torch_multiple_predictions_in_row(
    ...
    # Fixed expected values are covered by test_trt_package_torch above.
    # Here, verify that repeated calls return consistent predictions.
    expected = model(coins_counting_image_torch)[0]
    expected_confidence = expected.confidence.cpu()
    expected_class_id = expected.class_id.cpu()
    expected_xyxy = expected.xyxy.cpu()

    for _ in range(8):
        predictions = model(coins_counting_image_torch)[0]

        assert predictions.confidence.shape == expected_confidence.shape
        assert torch.allclose(
            predictions.confidence.cpu(), expected_confidence, atol=0.01
        )
        assert torch.equal(predictions.class_id.cpu(), expected_class_id)
        assert torch.allclose(predictions.xyxy.cpu(), expected_xyxy, atol=5)
