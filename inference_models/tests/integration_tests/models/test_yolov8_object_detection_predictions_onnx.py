...
def test_onnx_package_with_dynamic_batch_size_and_letterbox_batch_torch_list(
    ...
    # when
    predictions = model(
        [coins_counting_image_torch, coins_counting_image_torch],
        confidence=0.25,
        iou_threshold=0.45,
        max_detections=100,
        class_agnostic_nms=False,
...
def test_onnx_per_class_confidence_blocks_specific_class(
    ...
    above) detects 10 objects: one class 4, nine class 1. Setting a 0.99
    per-class threshold on class 1 should leave only the class 4 detection."""
    from inference_models.models.yolov8.yolov8_object_detection_onnx import (
        YOLOv8ForObjectDetectionOnnx,
    )
