...
@pytest.mark.parametrize(
    ...
    [
        (
            download.check_range_download_option,
            ("https://example.test/model.bin", 5, set()),
        ),
        (
            download.get_content_length,
            ("https://example.test/model.bin",),
        ),
        (
            download.download_chunk,
            ("https://example.test/model.bin", 0, 0, "/tmp/model.bin", set()),
        ),
        (
...
def test_low_level_download_helpers_reject_offline_network_access(
    network_helper,
    args,
) -> None:
    with mock.patch.object(download, "OFFLINE_MODE", True), mock.patch.object(
