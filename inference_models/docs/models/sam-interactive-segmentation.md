...
Segment Anything Model (SAM) is a foundation model for interactive image segmentation developed by Meta AI. It enables precise object segmentation using prompts like points, boxes, or masks.

## Overview

SAM provides interactive segmentation capabilities through a two-stage process:

- **Image embedding** - Encodes the image into a latent representation
- **Prompt-based segmentation** - Generates masks based on user prompts (points, boxes, or masks)
- **Caching support** - Embeddings and masks can be cached for faster repeated segmentation
- **Multi-mask output** - Can generate multiple mask proposals for ambiguous prompts
...
All SAM models require a **Roboflow API key**.

| Model Size | Model ID |
|------------|----------|
| ViT-B (Base) | `sam/vit_b` |
| ViT-L (Large) | `sam/vit_l` |
| ViT-H (Huge) | `sam/vit_h` |
...
model = AutoModel.from_pretrained("sam/vit_b", api_key="your_api_key")
...
Returns: `List[SAMPrediction]` (one per image in batch)

Each `SAMPrediction` contains:
...
SAM supports two types of caching for significant performance improvements:
...
```python
from inference_models import AutoModel
from inference_models.models.sam.cache import SamImageEmbeddingsInMemoryCache
import cv2
import numpy as np

# Create cache with size limit (number of images to cache)
embeddings_cache = SamImageEmbeddingsInMemoryCache.init(
...
model = AutoModel.from_pretrained(
    ...
    sam_image_embeddings_cache=embeddings_cache
)
...
Caches the low-resolution mask logits from previous segmentations for iterative refinement.

```python
from inference_models.models.sam.cache import SamLowResolutionMasksInMemoryCache

# Create mask cache
masks_cache = SamLowResolutionMasksInMemoryCache.init(
...
model = AutoModel.from_pretrained(
    ...
    api_key="your_api_key",
    sam_image_embeddings_cache=embeddings_cache,
    sam_low_resolution_masks_cache=masks_cache
)
