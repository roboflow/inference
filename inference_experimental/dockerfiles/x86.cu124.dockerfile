...
RUN rm -rf /var/lib/apt/lists/* && apt-get clean && apt-get update -y && DEBIAN_FRONTEND=noninteractive apt-get install -y \
    ...
    libxext6 \
    curl
RUN add-apt-repository -y ppa:deadsnakes/ppa && apt update -y && apt install -y python3.12 python3-devel
...
RUN WHEEL=$(ls dist/inference_exp-*.whl) && $HOME/.local/bin/uv pip install --system "${WHEEL}[torch-cu124,onnx-cu12,mediapipe,grounding-dino,trt10]"
