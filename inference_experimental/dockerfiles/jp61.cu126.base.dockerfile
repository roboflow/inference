...
RUN UV_PROJECT_ENVIRONMENT=$(dirname $(which python)) $HOME/.local/bin/uv sync --locked --no-install-package opencv-python --extra torch-jp6-cu126 --extra onnx-jp6-cu126
