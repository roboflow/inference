...
start_test_docker_cpu:
	docker run -d --rm -p $(PORT):$(PORT) -e PORT=$(PORT) -e MAX_BATCH_SIZE=17 -e USE_INFERENCE_MODELS=$(USE_INFERENCE_MODELS) --name inference-test roboflow/${INFERENCE_SERVER_REPO}:test

start_test_docker_gpu:
	docker run -d --rm -p $(PORT):$(PORT) -e PORT=$(PORT) -e MAX_BATCH_SIZE=17 -e USE_INFERENCE_MODELS=$(USE_INFERENCE_MODELS) --gpus=all --name inference-test roboflow/${INFERENCE_SERVER_REPO}:test
