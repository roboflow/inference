...
- OBS Connection block (`roboflow_core/obs_connection@v1`): the address of an OBS Studio websocket server, consumed by the OBS Action block. When no password is given and OBS runs on the same machine, the password is read from OBS's own config, only if `ALLOW_WORKFLOW_BLOCKS_ACCESSING_LOCAL_STORAGE` allows reading local files; `message` says where the credential came from, and the connection passed between steps never carries the password. Speaks the obs-websocket 5.x protocol (OBS 28 or later) directly over the existing `websocket-client` dependency. Not available on Hosted Serverless, Dedicated Deployments or with remote step execution.
- OBS Action block (`roboflow_core/obs_action@v1`): one OBS operation per run: switch scene, show or hide a source, set text, toggle a filter, trigger a hotkey, move or resize a source (including onto the most confident detection), start or stop the virtual camera or recording. Identical consecutive requests are skipped by default (`skip_if_unchanged`), a target that resolves to nothing leaves OBS as it is, and an unknown scene or source reports the names OBS has. Failures are reported in `error_status` and `message` instead of raised. Same runtime restrictions as the OBS Connection block.
- Detections Class Router block (`roboflow_core/detections_class_router@v1`): maps the most confident detection's class to a value through a `{class: value}` table, with a default when nothing is routed, for example choosing an OBS scene from what a model sees. Accepts both prediction representations.
- RTSP Stream Watcher block (`roboflow_core/rtsp_stream_watcher@v1`): watches another RTSP camera from inside a Workflow. A background reader keeps the stream drained, samples it every `check_interval_seconds` and, with `model_id` set, runs that model on the sample in its own thread, so the Workflow's own frame rate is unaffected. Each run returns the latest `frame` and `predictions`, blank while the stream is down or stale. Readers are shared per stream and model and stop when idle. The camera must be reachable from the machine running the Workflow, so the block is not available on Hosted Serverless or Dedicated Deployments.
