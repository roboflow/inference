...
??? "Class: `RichLabelVisualizationBlockV1`"
    ...
    Source:
    <a target="_blank" href="https://github.com/roboflow/inference/blob/main/inference/core/workflows/core_steps/visualizations/rich_label/v1.py">inference.core.workflows.core_steps.visualizations.rich_label.v1.RichLabelVisualizationBlockV1</a>
...
Use the following identifier in step `"type"` field: `roboflow_core/rich_label_visualization@v1`to add the block as
...
`Rich Label Visualization` in version `v1`  has.
...
???+ tip "Bindings"
    ...
    - input
        ...
        - `font_family` (*[`string`](../kinds/string.md)*): Font used to render label text. Pick from approved fonts shipped with `inference`. Arbitrary font files or URLs are not supported. Pick 'Noto Sans' for the broadest character coverage..
        - `text_size_mode` (*[`string`](../kinds/string.md)*): How label text size is chosen. 'Manual' uses Size directly. 'Automatic' picks a readable size from image resolution and treats Size as a multiplier around the 1080p baseline (14 pt)..
        - `font_size` (*[`integer`](../kinds/integer.md)*): Font size of the label text, in points. In Manual mode this is the rendered size. In Automatic mode this multiplies the resolution-derived baseline (14 pt at 1080p min dimension)..
        ...
        - `max_line_length` (*[`integer`](../kinds/integer.md)*): Maximum number of characters per line before the label text wraps. Leave empty to disable wrapping..
...
??? tip "Example JSON definition of step `Rich Label Visualization` in version `v1`"
