...
def get_full_type_name(selected_type: type) -> str:
    ...
    # Keep the historic dotted form so `fully_qualified_block_class_name` and
    # every user-visible spelling of a block class does not change when the
    # source moves under `roboflow_workflows`. `class.__module__` is left
    # canonical: rewriting it would break standalone pickling.
    return to_legacy_module(t_module) + "." + selected_type.__qualname__
