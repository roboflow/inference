...
def parse_multi_class_classification_results(
    ...
    try:
        ...
        for class_name, class_id in class2id_mapping.items():
            confidence_vector[class_id] = confidences.get(class_name, 0.0)
...
def parse_multi_label_classification_results(
    ...
    try:
        ...
        confidence_vector = [0.0] * num_classes
        for class_name, class_id in class2id_mapping.items():
            confidence_vector[class_id] = predicted_classes_confidences.get(
                class_name, 0.0
            )
        predicted_class_ids = [
            class2id_mapping[class_name]
            for class_name in predicted_classes_confidences.keys()
        ]
        image_metadata = {
            CLASS_NAMES_KEY: class_names,
            # Lane 1b: explicit style tag -> serialiser reproduces the vlm formatter shape.
