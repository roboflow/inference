...
from roboflow_workflows.execution_engine.constants import DETECTION_ID_KEY
...
_segmentation_json_schema_extra = {
    ...
    KIND_KEY: [TENSOR_NATIVE_INSTANCE_SEGMENTATION_PREDICTION_KIND.dict()],
...
class MaskEdgeSnapManifest(WorkflowBlockManifest):
    ...
    segmentation: Union[str, InstanceDetections] = Field(
        title="Segmentation",
    ...
    def validate_segmentation(
        cls, value: Union[str, InstanceDetections]
    ) -> Union[str, InstanceDetections]:
    ...
    def discover_work_operations(self) -> List[WorkOperation]:
        return [WorkOperation.IMAGE_FILTERING, WorkOperation.DETECTION_PROCESSING]
...
def refine_masks(
    ...
    # Read the input instance masks one-at-a-time as numpy so cv2/numpy edge math
    # operates on plain arrays (never materialising the native input to sv.Detections).
    num_detections = len(segmentation)
    has_masks = segmentation.mask is not None and num_detections > 0
    segmentation_masks = (
        [instance_mask_to_numpy(segmentation, i) for i in range(num_detections)]
        if has_masks
        else []
    )
    ...
    refined_detections = InstanceDetections(
