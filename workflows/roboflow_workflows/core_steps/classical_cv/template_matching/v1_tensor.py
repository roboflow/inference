...
def apply_template_matching(
    ...
    if apply_nms:
        # sv.Detections is used here purely as the NMS algorithm (mirroring the
        # numpy block's with_nms); the output is built natively below — no
        # sv.Detections is returned.
        nms_input = sv.Detections(
            xyxy=xyxy,
            confidence=confidence,
            class_id=np.zeros(len(xyxy), dtype=np.uint32),
        ).with_nms(threshold=nms_threshold)
        xyxy = nms_input.xyxy
        confidence = nms_input.confidence
    return _native_detections_from_boxes(
        xyxy=xyxy,
        confidence=confidence,
        image=image,
...
def _native_detections_from_boxes(
    xyxy: np.ndarray,
