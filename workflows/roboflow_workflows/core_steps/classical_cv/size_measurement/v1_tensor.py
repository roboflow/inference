...
def get_detection_dimensions(
    detection: Union[Detections, InstanceDetections], index: int
    ...
    """
    if isinstance(detection, InstanceDetections):
        mask = instance_mask_to_numpy(detection, index).astype(np.uint8)
        ...
        bbox = detection.xyxy[index].detach().to("cpu").numpy()
        w = bbox[2] - bbox[0]
        h = bbox[3] - bbox[1]
...
class SizeMeasurementBlockV1(WorkflowBlock):
    ...
    def run(
        self,
        reference_predictions: Union[list, Detections, InstanceDetections],
        object_predictions: Union[Detections, InstanceDetections],
        reference_dimensions: Union[str, Tuple[float, float], List[float]],
    ) -> BlockResult:
        ...
        if hasattr(reference_predictions, "confidence"):
            ...
            ref_index = int(
                np.argmax(reference_predictions.confidence.detach().to("cpu").numpy())
            )
