...
"""
    ...
    def discover_work_operations(self) -> List[WorkOperation]:
        return [WorkOperation.IMAGE_ANALYSIS, WorkOperation.TEMPORAL_BUFFERING]
    ...
    def run(
        ...
        **kwargs,
        ...
        if self.frame_count < history and suppress_first_detections:
            ...
            return {
                "motion": False,
                "detections": _native_detections_from_boxes(
                    xyxy_boxes=[],
                    image=image,
                ),
                "alarm": False,
...
def _native_detections_from_boxes(
    ...
    return Detections(
        xyxy=torch.as_tensor(
            np.asarray(xyxy_boxes, dtype=np.float32),
            dtype=torch.float32,
