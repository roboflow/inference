...
from roboflow_workflows.execution_engine.entities.types import FLOAT_KIND, Selector
...
def compute_detection_areas(
    ...
    for i in range(n):
        if isinstance(detections, InstanceDetections):
            count = cv.countNonZero(
                instance_mask_to_numpy(detections, i).astype(np.uint8)
            )
            if count > 0:
...
class MaskAreaMeasurementBlockV1(WorkflowBlock):
    ...
    def run(
        ...
        areas_converted = areas_px / scale
        ...
        for i in range(num_detections):
            bboxes_metadata[i][AREA_KEY_IN_SV_DETECTIONS] = float(areas_px[i])
            bboxes_metadata[i][AREA_CONVERTED_KEY_IN_SV_DETECTIONS] = float(
                areas_converted[i]
            )
        predictions.bboxes_metadata = bboxes_metadata
