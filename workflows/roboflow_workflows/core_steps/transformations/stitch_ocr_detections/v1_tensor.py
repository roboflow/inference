"""Tensor-native sibling of stitch_ocr_detections/v1.py.

Loaded INSTEAD of v1.py when ENABLE_TENSOR_DATA_REPRESENTATION is set. Reuses the
exact same class name / block type@version (StitchOCRDetectionsBlockV1,
``roboflow_core/stitch_ocr_detections@v1``) so it is a drop-in swap in loader.py.
...
Native-input deltas vs the numpy file:
...
- box coordinates are read via ``.detach().to("cpu").numpy()`` then rounded to int,
  matching ``detections.xyxy.round().astype(int)`` in the numpy file.

Everything downstream of the two reads (line grouping, sorting, separators,
delimiter joining) is byte-for-byte identical to v1.py.
"""
