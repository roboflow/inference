"""Tensor-native sibling of stitch_ocr_detections/v2.py.

Loaded INSTEAD of v2.py when ENABLE_TENSOR_DATA_REPRESENTATION is set. Reuses the
exact same class name / block type@version (StitchOCRDetectionsBlockV2,
``roboflow_core/stitch_ocr_detections@v2``) so it is a drop-in swap in loader.py.

This block is a CONSUMER only: it takes OCR object-detection predictions and emits
a stitched text string under ``ocr_text`` (a plain ``STRING_KIND`` output, NOT a
prediction kind), so there is nothing to rebuild natively on the produce side and
no run_remotely path on this block.

Native-input deltas vs the numpy file (applied identically to all three stitching
algorithms - tolerance, otsu, collimate):
- input is an ``inference_models.Detections`` (xyxy is a torch tensor; the legacy
  numpy block received an ``sv.Detections``);
- the recognised text rides per-box on ``bboxes_metadata[i]["text"]`` under the
  agreed easy_ocr / ocr convention (the numpy block read it from
  ``detections.data["class_name"]``);
- box coordinates are read via ``.detach().to("cpu").numpy()`` - the tolerance
  algorithm rounds-to-int (matching ``.round().astype(int)``), otsu / collimate
  keep float coordinates exactly as the numpy file did with the raw ``.xyxy``.

Everything downstream of those two reads (Otsu thresholding, collimate traversal,
line grouping, sorting, separators, delimiter joining) is byte-for-byte identical
to v2.py.
"""
...
class BlockManifest(WorkflowBlockManifest):
    ...
    def discover_work_operations(self) -> List[WorkOperation]:
        return [WorkOperation.DETECTION_PROCESSING, WorkOperation.DATA_TRANSFORMATION]
...
def _native_xyxy_float(detections: Detections) -> np.ndarray:
    """Native equivalent of the raw ``detections.xyxy`` numpy array used by the
    otsu / collimate algorithms: pull the torch xyxy tensor to CPU numpy, keeping
    float coordinates."""
    return detections.xyxy.detach().to("cpu").numpy()


def _native_xyxy_int(detections: Detections) -> np.ndarray:
    """Native equivalent of ``detections.xyxy.round().astype(int)`` used by the
    tolerance algorithm: pull to CPU numpy, round, cast to int."""
...
def _native_class_names(detections: Detections) -> np.ndarray:
    ...
    both fancy-indexing (tolerance) and scalar indexing (otsu / collimate) work
    exactly as in the numpy file."""
...
def stitch_ocr_detections(
    ...
    Args:
        detections: Native inference_models Detections object containing OCR results
    ...
    xyxy = _native_xyxy_int(detections)
...
def adaptive_word_grouping(
    ...
    xyxy = _native_xyxy_float(detections)
    class_names = _native_class_names(detections)
