...
# the stream is opened from the process running the Workflow, whatever the step
# execution mode, so only network reachability of the camera restricts the block.
# Same code and axes as the ONVIF camera and PLC restrictions.
RTSP_CAMERA_LAN_ACCESS_RESTRICTION = RuntimeRestriction(
...
def _to_block_predictions(
    image: WorkflowImageData, responses: List[dict], class_filter: Optional[List[str]]
) -> Any:
    """Turn the provider's response for one image into what downstream blocks expect.

    Same two paths as the Object Detection block: supervision by default, an
    ``inference_models`` ``Detections`` when the tensor representation is on.
    """
    if ENABLE_TENSOR_DATA_REPRESENTATION:
        # imported lazily: this path needs torch, which is optional
        ...
        if class_filter:
            detection_dicts = [
                d for d in detection_dicts if d.get(CLASS_NAME_KEY) in class_filter
            ]
        return native_detections_from_inference_predictions(
            image=image,
            predictions=detection_dicts,
            prediction_type=PREDICTION_TYPE,
            inference_id=response.get(INFERENCE_ID_KEY),
        )
    predictions = convert_inference_detections_batch_to_sv_detections(responses)
...
def _empty_predictions(image: WorkflowImageData) -> Any:
    """No detections, in whichever representation the workflow runs with."""
    if ENABLE_TENSOR_DATA_REPRESENTATION:
        return _to_block_predictions(image=image, responses=[], class_filter=None)
...
class BlockManifest(WorkflowBlockManifest):
    ...
    def discover_work_operations(self) -> List[WorkOperation]:
        # reading the stream is network I/O; the latest sample is held across runs
        operations = [WorkOperation.EXTERNAL_REQUEST, WorkOperation.TEMPORAL_BUFFERING]
        if self.model_id:
            operations.append(WorkOperation.MODEL_INFERENCE)
    ...
    def get_restrictions(cls) -> List[RuntimeRestriction]:
        return [RTSP_CAMERA_LAN_ACCESS_RESTRICTION]
...
class RTSPStreamWatcherBlockV1(WorkflowBlock):
    ...
    def __init__(self, model_manager: ModelsProvider, api_key: Optional[str]):
    ...
    def _build_detector(
        ...
        def detect(frame: np.ndarray) -> Any:
            ...
            predictions = self._model_manager.run_object_detection(
                model_id=model_id,
                images=[image.to_inference_format(numpy_preferred=True)],
                api_key=self._api_key,
                confidence=confidence,
                class_filter=class_filter,
                # the same defaults the Object Detection block sends; NMS-based
                # models need concrete values, DETR-style models ignore them
                class_agnostic_nms=False,
                iou_threshold=0.3,
                max_detections=300,
                max_candidates=3000,
            )
            return _to_block_predictions(
                image=image, responses=predictions, class_filter=class_filter
            )
    ...
    def run(
        ...
        return {
            ...
            "predictions": (
                ...
                else _empty_predictions(frame_image)
            ),
