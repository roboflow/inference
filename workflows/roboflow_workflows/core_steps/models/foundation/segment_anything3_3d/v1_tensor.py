"""Tensor-native sibling of `roboflow_core/segment_anything3_3d_objects@v1`.

SCRATCH — first pass for review. SAM3-3D has NO inference_models tensor path (it is
a request-based mesh / Gaussian-splat generator). So this sibling does NOT change
the model invocation — it only adapts the DATA REPRESENTATION at the block
boundary so the tensor-native pipeline can drive it:

- `mask_input` arrives as `inference_models.InstanceDetections` (the tensor-native
  instance-segmentation kind) instead of `sv.Detections`; `extract_masks_from_input`
  materialises a list of (H, W) binary numpy masks from it (RLE or dense), which is
  exactly what the numpy block produced from `sv.Detections.mask`.
- The IMAGE is consumed via `base64_image` / `to_inference_format` exactly as the
  numpy block — `WorkflowImageData` materialises those from the CHW tensor_image
  transparently; the request-based model needs base64/numpy, not a tensor, so there
  is no tensor image path to take here.
- Outputs (mesh_glb / gaussian_ply / objects / inference_time) are 3D artifacts,
  not predictions — unchanged.
"""
...
from roboflow_workflows.core_steps.common.tensor_native import instance_mask_to_numpy
...
Accepts masks as: tensor-native instance segmentation (InstanceDetections) or a
flat list of polygon coordinates in COCO format.
"""
...
class BlockManifest(WorkflowBlockManifest):
    ...
    mask_input: Selector(
        kind=[LIST_OF_VALUES_KIND, TENSOR_NATIVE_INSTANCE_SEGMENTATION_PREDICTION_KIND]
    ) = Field(
    ...
    def discover_portable_restrictions(self) -> List[RestrictionMetadata]:
        return [
            REQUIRES_GPU_FOR_LOCAL_EXECUTION,
            hosted_endpoint_disabled_by_flag("SAM3_3D_OBJECTS_ENABLED"),
        ]
...
class SegmentAnything3_3D_ObjectsBlockV1(WorkflowBlock):
    ...
    def run(
        ...
        if self._step_execution_mode is StepExecutionMode.LOCAL:
            return self.run_locally(images=images, mask_input=mask_input)
        elif self._step_execution_mode is StepExecutionMode.REMOTE:
            return self.run_remotely(images=images, mask_input=mask_input)
    ...
    def run_remotely(
        ...
        for single_image, single_mask_input in zip(images, mask_input):
            ...
            if isinstance(converted_mask, list):
                converted_mask = [
                    mask.tolist() if isinstance(mask, np.ndarray) else mask
...
def extract_masks_from_input(mask_input: Any) -> Any:
    """Materialise a list of (H, W) binary numpy masks from a tensor-native
    InstanceDetections (RLE or dense, via instance_mask_to_numpy); pass through
    other formats (e.g. a flat polygon list) unchanged.

    Matches the numpy block, which returned `list(sv.Detections.mask)`.
    """
    if isinstance(mask_input, InstanceDetections):
        number_of_instances = len(mask_input)
        if number_of_instances == 0:
            raise ValueError("InstanceDetections contains no detections.")
        return [
            instance_mask_to_numpy(mask_input, index)
