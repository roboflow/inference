...
from roboflow_workflows.core_steps.sinks.obs.client import (
    call_with_reconnect,
    register_connection,
)
from roboflow_workflows.core_steps.sinks.obs.discovery import (
    discover_password,
    is_local_host,
)
...
Discovery reads OBS's own config file from this machine, so it is only attempted when the
server allows blocks to read local storage (`ALLOW_WORKFLOW_BLOCKS_ACCESSING_LOCAL_STORAGE`,
on by default for self-hosted servers). With that flag off, supply the password explicitly.
...
Serverless and Dedicated Deployments cannot reach a local OBS instance. The block speaks the
obs-websocket 5.x protocol (OBS 28 or later) directly, so no extra package is needed.
"""
...
class BlockManifest(WorkflowBlockManifest):
    ...
    def get_restrictions(cls) -> List[RuntimeRestriction]:
        return [OBS_LAN_ACCESS_RESTRICTION]

    def discover_work_operations(self) -> List[WorkOperation]:
        operations = []
        if self.verify_connection is not False:
            operations.append(WorkOperation.EXTERNAL_REQUEST)
        if self.discover_password is not False:
            # password discovery reads OBS's own config file on this machine
            operations.append(WorkOperation.STORAGE_READ)
...
class OBSConnectionBlockV1(WorkflowBlock):
    ...
    def __init__(self, disable_sinks: bool = False):
        ...
        # A video workflow runs this block every frame; verification and config
        # discovery are done once per instance, not once per frame.
        self._verified_versions: Dict[Tuple[str, int], str] = {}
        self._discovered: Dict[str, Tuple[Optional[str], str]] = {}
    ...
    def _resolve_password(
        ...
        if not ALLOW_WORKFLOW_BLOCKS_ACCESSING_LOCAL_STORAGE:
            # discovery reads a file from the host, which is exactly what this
            # flag exists to forbid; never touch the disk when it is off
            return (
                password,
                "no password supplied; discovery is off because "
                "ALLOW_WORKFLOW_BLOCKS_ACCESSING_LOCAL_STORAGE is disabled, so the "
                "OBS config on this machine was not read",
            )
    ...
    def run(
        ...
        key = (host, port)
        if password or host not in self._discovered:
            resolved, credential_note = self._resolve_password(
                host=host, password=password, allow_discovery=discover_password
            )
            if not password:
                self._discovered[host] = (resolved, credential_note)
        else:
            resolved, credential_note = self._discovered[host]
        register_connection(host=host, port=port, password=resolved, timeout=timeout)
        # The descriptor deliberately omits the password: it can be wired to a
        # Workflow output, and credentials are held in-process by the client pool.
        connection = {"host": host, "port": port, "timeout": timeout}
        if self._disable_sinks:
            return {
                CONNECTION_OUTPUT_KEY: connection,
                "error_status": False,
                "message": "Sinks disabled by workflow execution policy; connection not verified",
                "obs_version": "",
            }
        ...
        if key in self._verified_versions:
            obs_version = self._verified_versions[key]
            return {
                CONNECTION_OUTPUT_KEY: connection,
                "error_status": False,
                "message": (
                    f"Connected to OBS Studio {obs_version} at {host}:{port} "
                    f"({credential_note}; verified earlier)"
                ),
        ...
        try:
            version = call_with_reconnect(
                host=host,
                port=port,
                operation=lambda client: client.call("GetVersion"),
            )
            obs_version = version.get("obsVersion", "") or ""
            self._verified_versions[key] = obs_version
            return {
                CONNECTION_OUTPUT_KEY: connection,
                "error_status": False,
                "message": (
            ...
            # forget any discovered credential so a fixed OBS config is re-read
            self._discovered.pop(host, None)
            return {
                CONNECTION_OUTPUT_KEY: connection,
                "error_status": True,
