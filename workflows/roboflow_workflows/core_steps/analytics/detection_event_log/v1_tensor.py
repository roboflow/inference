...
class DetectionEventLogBlockV1(WorkflowBlock):
    ...
    def run(
        ...
        Args:
            ...
            detections: Tracked tensor-native detections with tracker_id (in
                `bboxes_metadata`) from ByteTracker.
        ...
        # Process detections. Tensor-native predictions carry tracker_id in
        # per-detection `bboxes_metadata` and class names in `image_metadata`;
        # detections without a tracker_id are skipped.
        class_names_map = (detections.image_metadata or {}).get(CLASS_NAMES_KEY) or {}
        tracked_objects: List[Tuple[int, str]] = []
        for _xyxy, _mask, class_id, _conf, tracker_id, _data, _meta in detections:
            if tracker_id is None:
                continue
            class_id_int = int(class_id)
            class_name = class_names_map.get(class_id_int, f"class_{class_id_int}")
            tracked_objects.append((int(tracker_id), str(class_name)))
        ...
        for tracker_id, class_name in tracked_objects:
