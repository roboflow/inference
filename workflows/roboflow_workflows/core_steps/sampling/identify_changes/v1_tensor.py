...
class BlockManifest(WorkflowBlockManifest):
    ...
    embedding: Selector(kind=[TENSOR_NATIVE_EMBEDDING_KIND]) = Field(
    ...
    def describe_outputs(cls) -> List[OutputDefinition]:
        return [
            ...
            OutputDefinition(name="average", kind=[TENSOR_NATIVE_EMBEDDING_KIND]),
            OutputDefinition(name="std", kind=[TENSOR_NATIVE_EMBEDDING_KIND]),
    ...
    def discover_work_operations(self) -> List[WorkOperation]:
        return [WorkOperation.NUMERICAL_COMPUTATION, WorkOperation.TEMPORAL_BUFFERING]
...
class IdentifyChangesBlockV1(WorkflowBlock):
    ...
    def run(
        ...
        if self.average is not None:
            ...
            cs = np.float64(
                (
                    ...
                    / torch.sqrt(
                        torch.dot(embedding, embedding)
                        * torch.dot(self.average, self.average)
                    )
                ).item()
            )
        ...
        # update average and std (kept as torch tensors, on the embedding's device)
        if self.average is None:
            self.average = embedding
            self.std = torch.zeros_like(embedding)
            self.var = torch.zeros_like(embedding)
            self.M2 = torch.zeros_like(embedding)
        else:
            if strategy == "Exponential Moving Average (EMA)":
                # Update EMA average:
                self.average = (
                ...
                window = torch.stack(self.sliding_window)
                self.average = window.mean(dim=0)
                self.std = window.std(dim=0, unbiased=False)
