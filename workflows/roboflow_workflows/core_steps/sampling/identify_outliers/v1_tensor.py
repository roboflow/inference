...
class IdentifyOutliersBlockV1(WorkflowBlock):
    ...
    def _fit_vmf_parameters(self, embeddings: torch.Tensor):
        """
        Fit a von Mises-Fisher distribution to the given set of unit-normalized embeddings.
        Returns:
            mu (torch.Tensor): Mean direction vector.
    ...
    def run(
        ...
        # Tensor-native embeddings stay on-device: the vMF math runs in torch.
        ...
        all_emb_array = torch.stack(self.all_embeddings)
        ...
        t_new = torch.dot(mu, embedding_normed)
        ...
        t_values = all_emb_array @ mu
        # Sort t-values to find percentile
        sorted_t = torch.sort(t_values).values
        rank = int(torch.searchsorted(sorted_t, t_new, right=False))
