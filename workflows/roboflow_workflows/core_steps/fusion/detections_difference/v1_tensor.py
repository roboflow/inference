"""Tensor-native sibling of ``detections_difference/v1.py``.

Consumes / produces native ``inference_models`` detections (``Detections`` /
``InstanceDetections``) instead of ``sv.Detections``. The matching algorithm is
re-implemented verbatim from the numpy block: the pairwise cost model, the
greedy one-to-one assignment and the removed / persisted / new split are
byte-identical — only the data carrier differs. Geometry is materialised to
host numpy once per run (``xyxy`` is a handful of boxes, not image data), the
outputs are row-subsets of the inputs taken with ``take_detections_by_indices``
so per-box metadata (detection ids, lineage) and the per-image
``image_metadata`` survive for downstream serialisation.
"""
...
from roboflow_workflows.execution_engine.constants import CLASS_NAMES_KEY
...
class BlockManifest(WorkflowBlockManifest):
    ...
    def describe_outputs(cls) -> List[OutputDefinition]:
        return [
            OutputDefinition(
                name="removed_detections",
                kind=[TENSOR_NATIVE_OBJECT_DETECTION_PREDICTION_KIND],
            ),
            OutputDefinition(
                name="persisted_detections",
                kind=[TENSOR_NATIVE_OBJECT_DETECTION_PREDICTION_KIND],
            ),
            OutputDefinition(
                name="new_detections",
    ...
    def discover_work_operations(self) -> List[WorkOperation]:
        return [WorkOperation.DETECTION_PROCESSING, WorkOperation.DETECTION_MATCHING]
...
class DetectionsDifferenceBlockV1(WorkflowBlock):
    ...
    def run(
        self,
        reference_predictions: TensorNativeDetections,
        candidate_predictions: TensorNativeDetections,
...
def compute_pairwise_costs(
    reference: TensorNativeDetections,
    candidate: TensorNativeDetections,
    spatial_weight: float,
    class_mismatch_penalty: float,
    class_strict: bool,
) -> np.ndarray:
    ...
    """
    reference_xyxy = _xyxy_as_numpy(reference)
    candidate_xyxy = _xyxy_as_numpy(candidate)
    iou = sv.box_iou_batch(reference_xyxy, candidate_xyxy)
    spatial = 1.0 - iou
    disjoint = iou <= 0.0
    if np.any(disjoint):
        reference_centers = (reference_xyxy[:, :2] + reference_xyxy[:, 2:]) / 2
        candidate_centers = (candidate_xyxy[:, :2] + candidate_xyxy[:, 2:]) / 2
        deltas = reference_centers[:, None, :] - candidate_centers[None, :, :]
        center_distance = np.linalg.norm(deltas, axis=2)
        enclosing_x1 = np.minimum(
            reference_xyxy[:, None, 0], candidate_xyxy[None, :, 0]
        )
        enclosing_y1 = np.minimum(
            reference_xyxy[:, None, 1], candidate_xyxy[None, :, 1]
        )
        enclosing_x2 = np.maximum(
            reference_xyxy[:, None, 2], candidate_xyxy[None, :, 2]
        )
        enclosing_y2 = np.maximum(
            reference_xyxy[:, None, 3], candidate_xyxy[None, :, 3]
        )
...
def _xyxy_as_numpy(detections: TensorNativeDetections) -> np.ndarray:
    return detections.xyxy.detach().to("cpu").numpy().astype(float)


def _class_mismatch_matrix(
    reference: TensorNativeDetections, candidate: TensorNativeDetections
) -> Optional[np.ndarray]:
    ...
    Mirrors the numpy block's extractor chain: class names come from the
    per-image ``image_metadata`` class-names map (the tensor-native carrier of
    the numpy ``data["class_name"]`` column), class ids from ``class_id``.
    """
    extractors = (_class_name_labels, _class_id_labels)
...
def _class_name_labels(
    detections: TensorNativeDetections,
) -> Optional[List[str]]:
    """Per-detection class names resolved from the ``image_metadata`` class-names
    map, or ``None`` when the prediction carries no map (the numpy block's
    ``data.get(CLASS_NAME_DATA_FIELD) is None`` case)."""
...
def _class_id_labels(detections: TensorNativeDetections) -> Optional[np.ndarray]:
    if detections.class_id is None:
        return None
    return detections.class_id.detach().to("cpu").numpy()


def _subset(
    detections: TensorNativeDetections, indices: List[int]
) -> TensorNativeDetections:
    # Row-gather propagates the per-box `bboxes_metadata` (detection ids, parent
    # coordinates, ...) so downstream re-projection keeps working, and shares
    # `image_metadata` so even an EMPTY subset stays serialisable against its
    # source image (the numpy sibling returns `sv.Detections.empty()` there,
    # which has no image context to lose — the native carrier does).
    return take_detections_by_indices(detections, indices)
