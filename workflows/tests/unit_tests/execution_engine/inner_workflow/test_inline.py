"""
Unit tests for ``roboflow_workflows.execution_engine.v1.inner_workflow.inline``.
...
from roboflow_workflows.execution_engine.v1.inner_workflow.inline import (
...
_SCALAR_ONLY_ECHO_PLUGIN = "tests.stub_plugins.scalar_only_block_plugin"
...
def test_contains_inner_workflow_step_false_for_dispatched_workflow() -> None:
    steps = [
        {
            "name": "dispatch",
...
def test_contains_inner_workflow_step_false_for_unknown_mode() -> None:
    steps = [
        {
            "name": "typo",
            "type": USE_INNER_WORKFLOW_BLOCK_TYPE,
...
def test_parent_reference_rewrite_treats_dispatched_specification_as_opaque() -> None:
    dispatched = {
        "type": USE_INNER_WORKFLOW_BLOCK_TYPE,
        "name": "dispatch",
        "execution_mode": "remote_dispatch",
        "workflow_definition": {
            ...
            "steps": [
                {
                    "type": "example/block@v1",
                    "name": "local",
                    "value": "$steps.outer.result",
                }
            ],
            "outputs": [],
        },
        "parameter_bindings": {"value": "$steps.outer.result"},
    }

    rewritten = _replace_inner_step_control_and_output_refs_in_object(
        dispatched,
        inner_step_name="outer",
        output_name_to_selector={"result": "$steps.outer__child.output"},
        first_inlined_step_name="outer__child",
    )

    assert rewritten["parameter_bindings"]["value"] == "$steps.outer__child.output"
    assert (
        rewritten["workflow_definition"]["steps"][0]["value"] == "$steps.outer.result"
    )
...
def test_inline_inner_workflow_steps_expands_leaf_and_preserves_parent_inputs() -> None:
    ...
    with mock.patch.object(
        ...
        try:
            from roboflow_workflows.execution_engine.v1.inner_workflow.compiler_bridge import (
...
def test_inline_inner_workflow_steps_raises_when_inlining_makes_no_progress() -> None:
    ...
    with mock.patch.object(
        ...
        try:
            ...
            with mock.patch(
                "roboflow_workflows.execution_engine.v1.inner_workflow.inline._inline_one_inner_workflow_leaf",
                return_value=False,
            ):
                with pytest.raises(
