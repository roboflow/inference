...
from tests.unit_tests.execution_engine.introspection import (
...
def test_load_blocks_from_plugin_when_plugin_does_not_implement_interface() -> None:
    ...
    with pytest.raises(PluginInterfaceError):
        _ = load_blocks_from_plugin(
            "tests.unit_tests.execution_engine.introspection.plugin_with_initializers"
        )
...
def test_load_blocks_from_plugin_when_plugin_implement_interface() -> None:
    ...
    result = load_blocks_from_plugin(
        "tests.unit_tests.execution_engine.introspection.plugin_with_valid_blocks"
    )
...
def test_load_initializers_from_plugin_when_plugin_exists_and_initializers_provided() -> (
    ...
    assert (
        result[
            "tests.unit_tests.execution_engine.introspection.plugin_with_initializers.a"
        ]
        == 38
    ), "This parameter is expected to be static value"
    assert (
        result[
            "tests.unit_tests.execution_engine.introspection.plugin_with_initializers.b"
        ]()
        == 7
...
@mock.patch.dict(
    ...
    {
        "WORKFLOWS_PLUGINS": "tests.unit_tests.execution_engine.introspection.plugin_with_initializers"
    },
    clear=True,
)
...
def test_describe_available_blocks_when_valid_plugins_are_loaded_and_multiple_versions_in_the_same_family(
    ...
    # given
    load_workflow_blocks_mock.return_value = load_blocks_from_plugin(
        "tests.unit_tests.execution_engine.introspection.plugin_with_multiple_versions_of_blocks"
    )
...
def test_describe_available_blocks_when_plugins_duplicate_type_identifiers(
    ...
    # given
    load_workflow_blocks_mock.return_value = load_blocks_from_plugin(
        "tests.unit_tests.execution_engine.introspection.plugin_duplicated_identifiers"
    )
...
def test_load_kinds_serializers(
    ...
    get_plugin_modules_mock.return_value = [
        "tests.unit_tests.execution_engine.introspection.plugin_with_kinds_serializers"
    ]
