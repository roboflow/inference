...
`modal_executor.py` hides FOUR function-local `roboflow_workflows.environment` imports
...
def test_every_modal_name_is_imported_from_the_workflows_facade() -> None:
    imported = {
        ...
        if node.module == "roboflow_workflows.environment"
    ...
    for node in ast.walk(tree):
        ...
        for sub in ast.walk(node):
            if (
                ...
                and sub.module == "roboflow_workflows.environment"
            ):
                local += 1
