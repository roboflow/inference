...
from roboflow_workflows.core_steps.transformations.rtsp_stream_watcher import (
...
def test_manifest_defaults_are_demo_friendly():
    ...
    assert m.discover_dependent_resources() == []
...
def _fake_provider(class_name="dog"):
    """A ModelsProvider whose object detection answer is one box of `class_name`."""
    provider = MagicMock()
    provider.run_object_detection.return_value = [
        {
            "inference_id": "abc",
...
def test_background_model_output_is_supervision_by_default(monkeypatch):
    monkeypatch.setattr(watcher_module, "ENABLE_TENSOR_DATA_REPRESENTATION", False)
    block = RTSPStreamWatcherBlockV1(model_manager=_fake_provider(), api_key=None)
    detect = block._build_detector(
        model_id="m/1", confidence=0.4, class_filter=["dog"], parent_id="cam"
    )

    detections = detect(np.zeros((100, 100, 3), dtype=np.uint8))

    assert list(detections.data["class_name"]) == ["dog"]
    assert detections.xyxy.tolist() == [[40.0, 40.0, 60.0, 60.0]]


def test_background_model_output_follows_tensor_representation(monkeypatch):
    pytest.importorskip("torch")
    from inference_models.models.base.object_detection import Detections

    monkeypatch.setattr(watcher_module, "ENABLE_TENSOR_DATA_REPRESENTATION", True)
    block = RTSPStreamWatcherBlockV1(model_manager=_fake_provider(), api_key=None)
    detect = block._build_detector(
        model_id="m/1", confidence=0.4, class_filter=["dog"], parent_id="cam"
    )
    ...
    assert detections.xyxy.tolist() == [[40.0, 40.0, 60.0, 60.0]]
    assert detections.image_metadata["class_names"] == {3: "dog"}
    empty = watcher_module._empty_predictions(
        WorkflowImageData(
            parent_metadata=ImageParentMetadata(parent_id="cam"),
            numpy_image=np.zeros((10, 10, 3), dtype=np.uint8),
        )
    )
    assert isinstance(empty, Detections) and len(empty) == 0


def test_work_operations_include_model_inference_only_with_a_model():
    base = {
        "type": "roboflow_core/rtsp_stream_watcher@v1",
        "name": "cam",
        "stream_url": "rtsp://cam/live",
    }
    without_model = BlockManifest.model_validate(base).discover_work_operations()
    with_model = BlockManifest.model_validate(
        {**base, "model_id": "rfdetr-nano"}
    ).discover_work_operations()

    assert WorkOperation.MODEL_INFERENCE not in without_model
    assert WorkOperation.MODEL_INFERENCE in with_model
    assert WorkOperation.EXTERNAL_REQUEST in without_model
    assert WorkOperation.TEMPORAL_BUFFERING in without_model
