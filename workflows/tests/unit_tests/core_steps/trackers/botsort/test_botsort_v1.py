...
from tests.unit_tests.core_steps.trackers.conftest import (
    FRAME1_XYXY,
    FRAME2_XYXY,
    FRAME3_XYXY,
    make_detections,
...
def test_botsort_block() -> None:
    block = BoTSORTBlockV1()
    block.run(
...
def test_botsort_with_cmc() -> None:
    block = BoTSORTBlockV1()
    block.run(
        image=wrap_with_workflow_image(make_metadata(10)),
        detections=make_detections(FRAME1_XYXY),
    ...
    assert len(frame2_result["tracked_detections"]) == 3


def test_botsort_missing_fps() -> None:
...
def test_botsort_input_and_output_kinds_match() -> None:
    input_kinds = manifest_accepted_kind_names(BoTSORTManifest, field_name="detections")
    for output_def in BoTSORTManifest.describe_outputs():
...
def test_botsort_outputs_superset_of_predictions_visualization_kinds() -> None:
...
def test_botsort_not_video_file() -> None:
    block = BoTSORTBlockV1()
    block.run(
