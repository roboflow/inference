...
import numpy as np
import pytest
import roboflow_workflows.core_steps.visualizations.label.v1_tensor as label_v1_tensor
...
def test_cached_sprites_are_device_resident_tensors(device: str, monkeypatch) -> None:
    # given
    monkeypatch.setattr(
        "roboflow_workflows.execution_engine.entities.base.WORKFLOWS_IMAGE_TENSOR_DEVICE",
        torch.device(device),
    )
