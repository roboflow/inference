...
from roboflow_workflows.enterprise_blocks.sinks.postgresql import v1
...
def test_permissive_mode_does_not_resolve_or_pin(connect, monkeypatch):
    ...
    monkeypatch.setattr(
        v1, "ALLOW_POSTGRESQL_WORKFLOWS_SINK_TO_NON_GLOBAL_ADDRESSES", True
    )
    resolver = MagicMock()
...
def test_restrictive_mode_pins_validated_ip(connect, monkeypatch):
    monkeypatch.setattr(
        v1, "ALLOW_POSTGRESQL_WORKFLOWS_SINK_TO_NON_GLOBAL_ADDRESSES", False
    )
    monkeypatch.setattr(
...
def test_denylisted_reason_blocks_resolved_ip():
    ...
    assert (
        reason
        == "host 'db.example.com' resolves to a denylisted address '93.184.216.34'"
    )
...
def test_denylisted_reason_none_when_clear():
    ...
    assert (
        v1.denylisted_reason("db.example.com", ["93.184.216.34"], {"1.2.3.4"}) is None
    )
...
def test_local_resolver_is_faithful_copy(monkeypatch):
    ...
    monkeypatch.setattr(
        v1.socket,
        "getaddrinfo",
        MagicMock(return_value=[(0, 0, 0, "", ("93.184.216.34", 5432))]),
    )
    ...
    monkeypatch.setattr(
        v1.socket,
        "getaddrinfo",
        MagicMock(return_value=[(0, 0, 0, "", ("127.0.0.1", 5432))]),
    )
...
def test_denylist_active_even_when_non_global_allowed(connect, monkeypatch):
    ...
    monkeypatch.setattr(
        v1, "POSTGRESQL_WORKFLOWS_SINK_BLACKLISTED_ADDRESSES", {"93.184.216.34"}
    )
    monkeypatch.setattr(
        v1.socket,
        "getaddrinfo",
...
def test_not_whitelisted_reason_allows_host_match():
    assert (
        v1.not_whitelisted_reason(
            "db.example.com", ["93.184.216.34"], {"db.example.com"}
        )
        is None
    )
...
def test_allowlist_blocks_non_listed_host_end_to_end(connect, monkeypatch):
    ...
    monkeypatch.setattr(
        v1, "POSTGRESQL_WORKFLOWS_SINK_WHITELISTED_ADDRESSES", {"db.allowed.com"}
    )
    monkeypatch.setattr(
        v1.socket,
        "getaddrinfo",
