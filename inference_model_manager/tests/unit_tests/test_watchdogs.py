"""Unit tests for inference_model_manager.watchdogs.

Merged from the legacy inference_models_cache_watchdog and cuda_memory_watchdog
test suites. torch (and a CUDA device) is not available in the unit-test
environment, so the CUDA tests stub ``torch`` via ``sys.modules`` or patch the
module-level ``cuda_is_available`` / ``reclaim_cuda_memory`` helpers directly.
"""
...
from inference_model_manager import watchdogs as mod
...
def test_second_start_while_running_does_not_probe_cuda() -> None:
    with mock.patch.object(
        mod, "cuda_is_available", return_value=True
    ) as mocked_cuda_check, mock.patch.object(
        mod, "reclaim_cuda_memory", return_value=None
    ):
        wd = CudaMemoryReclamationWatchdog(interval_seconds=5.0)
        wd.start()
        try:
            mocked_cuda_check.reset_mock()
            wd.start()
            mocked_cuda_check.assert_not_called()
        finally:
            wd.stop(timeout=1.0)
...
class TestStartEnabledWatchdogs:
    def test_all_disabled_by_default(self, monkeypatch):
        import inference_model_manager.configuration as cfg
        from inference_model_manager import watchdogs

        monkeypatch.setattr(cfg, "MAX_INFERENCE_MODELS_CACHE_SIZE_MB", -1)
        monkeypatch.setattr(cfg, "ENABLE_CUDA_MEMORY_RECLAMATION_WATCHDOG", False)
        assert watchdogs.start_enabled_watchdogs() == []

    def test_cache_watchdog_started_when_sized(self, monkeypatch, tmp_path):
        import inference_model_manager.configuration as cfg
        from inference_model_manager import watchdogs

        monkeypatch.setattr(cfg, "MAX_INFERENCE_MODELS_CACHE_SIZE_MB", 100)
        monkeypatch.setattr(cfg, "ENABLE_CUDA_MEMORY_RECLAMATION_WATCHDOG", False)
        monkeypatch.setattr(
            "inference_models.configuration.INFERENCE_HOME", str(tmp_path)
        )
        started = watchdogs.start_enabled_watchdogs()
        try:
