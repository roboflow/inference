...
10a. **BLOCK — Credential-bearing log interpolation:** any log line in the camera/producer stack (`inference/core/interfaces/camera/**`) interpolating a source reference, GStreamer launch string, or exception repr routes through `sanitize_source_reference` / `redact_credentials_in_text` (`camera/source_reference_sanitizer.py`) — raw interpolation leaked credentialed `rtsp://user:password@...` URLs at INFO *and* default WARNING level before the 2026-08 sweep; exception reprs embed the full launch string, so they need the text redactor too. Convention: producer selection logs positively at INFO, fallbacks at WARNING — both sanitized.
...
- **Runtime NVIDIA libraries need loader-cache resolution, not env-only.** Two recurring classes from the GPU/media images: (1) GStreamer plugins dlopen UNVERSIONED sonames (`libnvrtc.so`, `libnvjpeg.so`) that CUDA *runtime* base images don't ship — the fallback keyed on the HOST driver's CUDA major masks the gap on same-major machines and breaks on newer drivers; the fix pattern is an unversioned symlink + build-time `ctypes.CDLL` assertion in the Dockerfile. (2) Libraries in custom prefixes (`/opt/gstreamer` etc.) reachable only via the image's `LD_LIBRARY_PATH` die in Kubernetes, which REPLACES that variable (NVIDIA device-plugin injection) — new runtime lib prefixes need an `/etc/ld.so.conf.d/` entry + `ldconfig`, not just the ENV line.
