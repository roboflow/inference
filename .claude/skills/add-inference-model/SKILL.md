...
description: Add a new core (pre-trained, non-user-fine-tuned) model to the inference repos. Covers the inference_models class + registry entry + tests and the optional surfaces (workflow block, legacy-endpoint adapter). Weight packaging and registration into the Roboflow model registry are handled by internal Roboflow Inference Core team tooling and are out of scope here. Trigger when the user asks to "add a model", "port a new model from HuggingFace", "wrap a transformers model", "expose X as a workflow block / model", or similar. This is a living skill — iterate on it each time a new model ships.
---
...
| 2 | Weight packaging + registry registration | **Always** (so the model is actually loadable) | Internal tooling maintained by the Roboflow Inference Core team — see "Surface 2" (step 6) below |
...
### 6. Surface 2 — weight packaging & registry registration (internal tooling)
...
Once the model has been registered by the internal tooling, the integration/e2e tests (steps 4 and 8) and `AutoModel.from_pretrained("<arch>/<variant>")` can load it.

### 7. Inference-models adapter (surface 4, optional)
...
Once the model has been registered (via the internal tooling, step 6), load it and exercise it end to end:

- `AutoModel.from_pretrained("<arch>/<variant>")` with a real input.
...
- **`PYTHONSAFEPATH=1`** when running scripts from the repo root — see step 8.
