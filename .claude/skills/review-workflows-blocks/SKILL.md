...
description: "Review PRs touching workflows/roboflow_workflows/core_steps/** (Workflow blocks — {category}/{block}/vN.py, common/, common/query_language/**, loader.py) or workflows/roboflow_workflows/execution_engine/entities/types.py (new kind). Diff signals — new block package, new/changed vN.py, new type Literal or legacy alias, describe_outputs()/run() changes, load_blocks()/load_kinds()/KINDS_SERIALIZERS edits, new *_KIND, sv.Detections.data writes, serializers/deserializers changes."
---
...
- `workflows/roboflow_workflows/core_steps/**` — Roboflow Core plugin blocks (`{category}/{block}/v{N}.py`, `common/`, `common/query_language/**`, `loader.py`).
- `workflows/roboflow_workflows/execution_engine/entities/types.py` — only when a NEW **kind** is added for a block (review it alongside the block).

OUT of scope (other skills own these): Execution Engine internals under `workflows/roboflow_workflows/execution_engine/v1/**` (compiler, executor, output_constructor — `review-workflows-execution-engine`); model backends under `inference_models/`. Since the tensor-data-representation merge (#2357), the `vN_tensor.py` siblings ARE in scope here — review them with the *Tensor-native parity* items below; authoring-side counterpart: `create-workflow-block`. UQL / `common/query_language/**` has no dedicated skill, so it IS reviewed here (see the query-language standard below).
...
6. **FLAG — Tests**: unit test under `workflows/tests/unit_tests/core_steps/{category}/`; integration test under `tests/workflows/integration_tests/execution/test_workflow_with_*_block.py` for new or output-changing blocks.
7. **FLAG — EE compatibility floor**: actively verify `get_execution_engine_compatibility()` against the EE capabilities the block actually uses — contributors habitually leave the copy-pasted default range unexamined. Derive the true minimum from the `### Execution engine` subsection of `workflows/CHANGELOG.md` and require the floor raised when the block relies on a capability introduced after the declared minimum. If a capability the block relies on exists only under `## Unreleased` (including one added in the same PR), the EE version must be placed and bumped so the block can gate on it — see the EE-compatibility standard.
8. **FLAG — EE behavior change**: if the block forces an EE behavior change, require a user-facing entry under `## Unreleased` in the `### Execution engine` subsection of `workflows/CHANGELOG.md` and surface the maintainer notice **Execution Engine requires a version bump for release** — never ask the contributor to bump `EXECUTION_ENGINE_V1_VERSION` or update version-assert tests (maintainers do both in the release PR), except in the unreleased-dependency case above. Outside that exception, a contributor PR touching that constant is a red flag.
...
- **EE compatibility**: `get_execution_engine_compatibility()` declares the minimum engine version a block needs, not its package version. Derive the floor from the engine capability history; do not raise it just because a newer engine exists. Engine behavior changes require one entry under `Unreleased` → `Execution engine` in `workflows/CHANGELOG.md`. Follow `.cursor/rules/execution-engine-version-changelog.mdc` for maintainer release responsibilities and the exception for a block depending on an unreleased capability. That exception assigns the engine version early while package notes remain under `Unreleased`.
...
- **Engine subsection** — ONLY when the block forces an EE behavior change: add an entry under `## Unreleased` in the `### Execution engine` subsection of `workflows/CHANGELOG.md`; maintainers bump `EXECUTION_ENGINE_V1_VERSION` at release time.

## Key files & entry points
- `workflows/roboflow_workflows/core_steps/loader.py` — `load_blocks()`, `load_kinds()`, `KINDS_SERIALIZERS`/`KINDS_DESERIALIZERS`, `REGISTERED_INITIALIZERS`.
- `workflows/roboflow_workflows/core_steps/{category}/{block}/vN.py` — block modules (manifest + block class).
- `workflows/roboflow_workflows/core_steps/common/{serializers,deserializers,utils}.py` — `sv.Detections` <-> wire conversion, keypoints/mask helpers.
- `workflows/roboflow_workflows/core_steps/common/query_language/**` — UQL operations/evaluation engine.
- `workflows/roboflow_workflows/execution_engine/entities/types.py` — `Kind` definitions + `*_KIND_DOCS`.
- `workflows/roboflow_workflows/execution_engine/entities/base.py` — `WorkflowImageData`, `Batch`, `OutputDefinition`.
- `workflows/roboflow_workflows/prototypes/block.py` — `WorkflowBlock`, `WorkflowBlockManifest`, `BlockResult`.
- `workflows/roboflow_workflows/execution_engine/v1/core.py` — `EXECUTION_ENGINE_V1_VERSION`.
- Docs: the Workflows developer guide on docs.roboflow.com (create a block, versioning, block bundling, EE changelog): <https://docs.roboflow.com/workflows/developer-guide/developer-guide>.
- Tests: `workflows/tests/unit_tests/core_steps/**`, `tests/workflows/integration_tests/execution/**`, `test_init_files.py`.
