...
## Required companions
- **Version bump (coordinated with other changes in release)** Nearly every merged PR here bumps `inference/core/version.py` `__version__` (e.g. #2399 `1.3.0`→`1.3.1`, #1051, #891). A functional CLI change with no version bump should be flagged, but can be dismissed by contributor in comment, given that there may be parallel changes to the same release. `inference_cli/version.py` also carries a `__version__` — but this is automatically adjusted in PyPI package build CI action.
...
- **CI requirements.** If a change pulls in new runtime imports, the unit-test CI install list must cover them. `inference` package is optional dependency - so typically, if needed, new deps also belong in `requirements/requirements.cli.txt`.
