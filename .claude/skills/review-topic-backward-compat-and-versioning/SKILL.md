...
- Changes a Workflow **block** manifest under `workflows/roboflow_workflows/core_steps/**` — its `type` Literal, I/O fields, `kinds`, defaults, or `get_execution_engine_compatibility`.
- Changes Workflow **compile/run behavior** under `workflows/roboflow_workflows/execution_engine/**`.
...
- **BLOCK** — EE compile/run behavior changed but no user-facing entry was added under `## Unreleased` in the `### Execution engine` subsection of `workflows/CHANGELOG.md`. (§3)
...
- `workflows/roboflow_workflows/execution_engine/v1/core.py` — `EXECUTION_ENGINE_V1_VERSION` constant that `get_execution_engine_compatibility` gates against.
- the `### Execution engine` subsection of `workflows/CHANGELOG.md` — canonical EE changelog format.
...
- `workflows/roboflow_workflows/core_steps/models/roboflow/instance_segmentation/{v1,v2,v3,v4}.py` — canonical additive block versioning; distinct `type` Literal + version-range compat per file.
