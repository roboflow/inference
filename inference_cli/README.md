...
- Active learning
- Commercial license for self-hosted deployment, available as an Enterprise add-on. A custom contract can include it as well. Without that add-on or contract, self-hosted deployment follows the model's own license. An AGPL-3.0 model must be used under AGPL-3.0.
